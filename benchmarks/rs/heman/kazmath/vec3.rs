...
pub unsafe extern "C" fn kmVec3Normalize(
    ...
    let mut l: libc::c_float = 1.0f32 / kmVec3Length(pIn);
    let mut v: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    v.x = (*pIn).x * l;
    v.y = (*pIn).y * l;
    v.z = (*pIn).z * l;
    (*pOut).x = v.x;
...
pub unsafe extern "C" fn kmVec3GetHorizontalAngle(
    ...
    let z1: libc::c_float = sqrt(
        ((*pIn).x * (*pIn).x + (*pIn).z * (*pIn).z) as libc::c_double,
    ) as libc::c_float;
    (*pOut)
...
pub unsafe extern "C" fn kmVec3RotationToDirection(
    ...
    let xr: libc::c_float = kmDegreesToRadians((*pIn).x);
    let yr: libc::c_float = kmDegreesToRadians((*pIn).y);
    let zr: libc::c_float = kmDegreesToRadians((*pIn).z);
    let cr: libc::c_float = cos(xr as libc::c_double) as libc::c_float;
    let sr: libc::c_float = sin(xr as libc::c_double) as libc::c_float;
    let cp: libc::c_float = cos(yr as libc::c_double) as libc::c_float;
    let sp: libc::c_float = sin(yr as libc::c_double) as libc::c_float;
    let cy: libc::c_float = cos(zr as libc::c_double) as libc::c_float;
    let sy: libc::c_float = sin(zr as libc::c_double) as libc::c_float;
    let srsp: libc::c_float = sr * sp;
    let crsp: libc::c_float = cr * sp;
...
pub unsafe extern "C" fn kmVec3Reflect(
    ...
    let mut tmp: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
