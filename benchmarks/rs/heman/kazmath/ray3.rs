...
pub unsafe extern "C" fn kmRay3IntersectPlane(
    ...
    let mut d: libc::c_float = (*plane).a * (*ray).dir.x + (*plane).b * (*ray).dir.y
        + (*plane).c * (*ray).dir.z;
    if d == 0 as libc::c_int as libc::c_float {
        return 0 as libc::c_int as libc::c_uchar;
    }
    let mut t: libc::c_float = -((*plane).a * (*ray).start.x
        + (*plane).b * (*ray).start.y + (*plane).c * (*ray).start.z + (*plane).d) / d;
    if t < 0 as libc::c_int as libc::c_float {
        return 0 as libc::c_int as libc::c_uchar;
    }
    let mut scaled_dir: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
