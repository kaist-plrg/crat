...
pub unsafe extern "C" fn kmMat4Identity(mut pOut: *mut kmMat4) -> *mut kmMat4 {
    ...
    (*pOut).mat[10 as libc::c_int as usize] = (*pOut).mat[15 as libc::c_int as usize];
    (*pOut).mat[5 as libc::c_int as usize] = (*pOut).mat[10 as libc::c_int as usize];
    (*pOut).mat[0 as libc::c_int as usize] = (*pOut).mat[5 as libc::c_int as usize];
    return pOut;
}
...
pub unsafe extern "C" fn kmMat4Multiply(
    ...
    let mut m1: *const libc::c_float = ((*pM1).mat).as_ptr();
    let mut m2: *const libc::c_float = ((*pM2).mat).as_ptr();
    mat[0 as libc::c_int
...
pub unsafe extern "C" fn kmMat4Assign(
    ...
    'c_6386: {
        if pOut != pIn as *mut kmMat4
            && !(b"You have tried to self-assign!!\0" as *const u8
                as *const libc::c_char)
                .is_null()
        {} else {
            __assert_fail(
...
pub unsafe extern "C" fn kmMat4AreEqual(
    ...
    'c_6697: {
        if pMat1 != pMat2
            && !(b"You are comparing the same thing!\0" as *const u8
                as *const libc::c_char)
                .is_null()
        {} else {
            __assert_fail(
...
pub unsafe extern "C" fn kmMat4RotationQuaternion(
    ...
    let mut xx: libc::c_double = ((*pQ).x * (*pQ).x) as libc::c_double;
    let mut xy: libc::c_double = ((*pQ).x * (*pQ).y) as libc::c_double;
    let mut xz: libc::c_double = ((*pQ).x * (*pQ).z) as libc::c_double;
    let mut xw: libc::c_double = ((*pQ).x * (*pQ).w) as libc::c_double;
    let mut yy: libc::c_double = ((*pQ).y * (*pQ).y) as libc::c_double;
    let mut yz: libc::c_double = ((*pQ).y * (*pQ).z) as libc::c_double;
    let mut yw: libc::c_double = ((*pQ).y * (*pQ).w) as libc::c_double;
    let mut zz: libc::c_double = ((*pQ).z * (*pQ).z) as libc::c_double;
    let mut zw: libc::c_double = ((*pQ).z * (*pQ).w) as libc::c_double;
    (*pOut)
        .mat[0 as libc::c_int
...
pub unsafe extern "C" fn kmMat4PerspectiveProjection(
    ...
    let mut r: libc::c_float = kmDegreesToRadians(
        fovY / 2 as libc::c_int as libc::c_float,
    );
    let mut deltaZ: libc::c_float = zFar - zNear;
    let mut s: libc::c_float = sin(r as libc::c_double) as libc::c_float;
    let mut cotangent: libc::c_float = 0 as libc::c_int as libc::c_float;
...
pub unsafe extern "C" fn kmMat4OrthographicProjection(
    ...
    let mut tx: libc::c_float = -((right + left) / (right - left));
    let mut ty: libc::c_float = -((top + bottom) / (top - bottom));
    let mut tz: libc::c_float = -((farVal + nearVal) / (farVal - nearVal));
    kmMat4Identity(pOut);
    (*pOut)
        .mat[0 as libc::c_int
...
pub unsafe extern "C" fn kmMat4LookAt(
    ...
    let mut f: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    let mut up: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    let mut s: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    let mut u: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
