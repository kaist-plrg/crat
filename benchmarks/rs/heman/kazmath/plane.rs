...
pub unsafe extern "C" fn kmPlaneFromPoints(
    ...
    let mut n: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
...
pub unsafe extern "C" fn kmPlaneIntersectLine(
    ...
    n.z = (*pP).c;
    kmVec3Normalize(&mut n, &mut n);
    let mut nt: libc::c_float = -(n.x * (*pV1).x + n.y * (*pV1).y + n.z * (*pV1).z
        + (*pP).d);
    let mut dt: libc::c_float = n.x * d.x + n.y * d.y + n.z * d.z;
    if fabs(dt as libc::c_double) < 0.0001f64 {
        pOut = 0 as *mut kmVec3;
        return pOut;
    }
    let mut t: libc::c_float = nt / dt;
    (*pOut).x = (*pV1).x + d.x * t;
    (*pOut).y = (*pV1).y + d.y * t;
    (*pOut).z = (*pV1).z + d.z * t;
    return pOut;
...
pub unsafe extern "C" fn kmPlaneNormalize(
    ...
    let mut n: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    let mut l: libc::c_float = 0 as libc::c_int as libc::c_float;
...
pub unsafe extern "C" fn kmPlaneClassifyPoint(
    ...
    let mut distance: libc::c_float = (*pIn).a * (*pP).x + (*pIn).b * (*pP).y
        + (*pIn).c * (*pP).z + (*pIn).d;
...
pub unsafe extern "C" fn kmPlaneExtractFromMat4(
    ...
    let mut scale: libc::c_int = if row < 0 as libc::c_int {
        -(1 as libc::c_int)
    } else {
        1 as libc::c_int
    };
...
pub unsafe extern "C" fn kmPlaneGetIntersection(
    ...
    let mut n1: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    let mut n2: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    let mut n3: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    let mut cross: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    let mut r1: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    let mut r2: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    let mut r3: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    let mut denom: libc::c_double = 0 as libc::c_int as libc::c_double;
