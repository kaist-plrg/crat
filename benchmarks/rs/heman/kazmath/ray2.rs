...
pub unsafe extern "C" fn kmLine2WithLineIntersection(
    ...
    let mut x1: libc::c_float = (*ptA).x;
    let mut y1: libc::c_float = (*ptA).y;
    let mut x2: libc::c_float = x1 + (*vecA).x;
    let mut y2: libc::c_float = y1 + (*vecA).y;
    let mut x3: libc::c_float = (*ptB).x;
    let mut y3: libc::c_float = (*ptB).y;
    let mut x4: libc::c_float = x3 + (*vecB).x;
    let mut y4: libc::c_float = y3 + (*vecB).y;
    let mut denom: libc::c_float = (y4 - y3) * (x2 - x1) - (x4 - x3) * (y2 - y1);
    if denom as libc::c_double > -0.0001f64 && (denom as libc::c_double) < 0.0001f64 {
        return 0 as libc::c_int as libc::c_uchar;
    }
    let mut ua: libc::c_float = ((x4 - x3) * (y1 - y3) - (y4 - y3) * (x1 - x3)) / denom;
    let mut ub: libc::c_float = ((x2 - x1) * (y1 - y3) - (y2 - y1) * (x1 - x3)) / denom;
    let mut x: libc::c_float = x1 + ua * (x2 - x1);
    let mut y: libc::c_float = y1 + ua * (y2 - y1);
    if !outTA.is_null() {
        *outTA = ua;
    }
    if !outTB.is_null() {
...
pub unsafe extern "C" fn calculate_line_normal(
    ...
    n.x = edge.y;
    n.y = -edge.x;
    let mut d: libc::c_float = kmVec2Dot(&mut n, &mut other_edge);
    if d > 0.0f32 {
        n.x = -n.x;
        n.y = -n.y;
    }
...
pub unsafe extern "C" fn kmRay2IntersectTriangle(
    ...
    let mut distance: libc::c_float = 10000.0f32;
    let mut intersected: libc::c_uchar = 0 as libc::c_int as libc::c_uchar;
    if kmRay2IntersectLineSegment(ray, p1, p2, &mut intersect) != 0 {
        let mut tmp: kmVec2 = kmVec2 { x: 0., y: 0. };
        let mut this_distance: libc::c_float = kmVec2Length(
    ...
    if kmRay2IntersectLineSegment(ray, p2, p3, &mut intersect) != 0 {
        ...
        let mut this_distance_0: libc::c_float = kmVec2Length(
    ...
    if kmRay2IntersectLineSegment(ray, p3, p1, &mut intersect) != 0 {
        ...
        let mut this_distance_1: libc::c_float = kmVec2Length(
...
pub unsafe extern "C" fn kmRay2IntersectBox(
    ...
    while i < 4 as libc::c_int as libc::c_uint {
        ...
        let mut next_point: *const kmVec2 = if i == 3 as libc::c_int as libc::c_uint {
            points[0 as libc::c_int as usize]
        } else {
            points[i.wrapping_add(1 as libc::c_int as libc::c_uint) as usize]
        };
        let mut other_point: *const kmVec2 = if i == 3 as libc::c_int as libc::c_uint
