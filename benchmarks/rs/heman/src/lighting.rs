...
pub unsafe extern "C" fn heman_lighting_compute_normals(
    ...
    while y < height {
        ...
        let mut px: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
        let mut py: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
...
pub unsafe extern "C" fn heman_lighting_apply(
    ...
    let mut width: libc::c_int = (*heightmap).width;
    let mut height: libc::c_int = (*heightmap).height;
    let mut final_0: *mut heman_image = heman_image_create(
        width,
        height,
        3 as libc::c_int,
    );
    ...
    let mut L: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    ...
    while y < height {
        ...
        while x < width {
            ...
            let mut df: libc::c_float = 1 as libc::c_int as libc::c_float
                - diffuse
            ...
            let mut of: libc::c_float = 1 as libc::c_int as libc::c_float
                - occlusion
                    * (1 as libc::c_int as libc::c_float
            ...
            if !albedo.is_null() {
                *color = *(heman_image_texel(albedo, x, y) as *mut kmVec3);
                ...
                (*color).z = 1 as libc::c_int as libc::c_float;
                (*color).y = (*color).z;
                (*color).x = (*color).y;
            }
            (*color)
...
unsafe extern "C" fn azimuth_slope(mut a: kmVec3, mut b: kmVec3) -> libc::c_float {
    let mut d: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    kmVec3Subtract(&mut d, &mut a, &mut b);
    let mut x: libc::c_float = kmVec3Length(&mut d);
    let mut y: libc::c_float = b.z - a.z;
    return y / x;
}
unsafe extern "C" fn compute_occlusion(
    ...
    let mut direction: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
    kmVec3Subtract(&mut direction, &mut horizonpt, &mut thispt);
    kmVec3Normalize(&mut direction, &mut direction);
    let mut dot: libc::c_float = kmVec3Dot(&mut direction, &KM_VEC3_POS_Z);
...
unsafe extern "C" fn horizon_scan(
    ...
    let mut sx: libc::c_int = (dx > 0 as libc::c_int) as libc::c_int
        - (dx < 0 as libc::c_int) as libc::c_int;
    let mut sy: libc::c_int = (dy > 0 as libc::c_int) as libc::c_int
        - (dy < 0 as libc::c_int) as libc::c_int;
    let mut ax: libc::c_int = abs(dx);
    let mut ay: libc::c_int = abs(dy);
    let mut nsweeps: libc::c_int = ay * w + ax * h - (ax + ay - 1 as libc::c_int);
    let mut p: *mut libc::c_int = startpts;
    let mut x: libc::c_int = -ax;
    while x < w - ax {
        let mut y: libc::c_int = -ay;
        while y < h - ay {
            if !(x >= 0 as libc::c_int && x < w && y >= 0 as libc::c_int && y < h) {
                let fresh0 = p;
                p = p.offset(1);
                *fresh0 = if sx < 0 as libc::c_int {
                    w - x - 1 as libc::c_int
                } else {
                    x
                };
    ...
    let mut pathlen: libc::c_int = 0 as libc::c_int;
    let mut i: libc::c_int = *startpts.offset(0 as libc::c_int as isize);
    let mut j: libc::c_int = *startpts.offset(1 as libc::c_int as isize);
    loop {
        i += dx;
        j += dy;
        pathlen += 1;
    ...
    let mut cellw: libc::c_float = _occlusion_scale
        / (if w > h { w } else { h }) as libc::c_float;
    let mut cellh: libc::c_float = _occlusion_scale
        / (if w > h { w } else { h }) as libc::c_float;
    ...
    while sweep < nsweeps {
        let mut convex_hull: *mut kmVec3 = hull_buffer
            .offset((sweep * pathlen) as isize);
        let mut p_0: *mut libc::c_int = startpts
            .offset((sweep * 2 as libc::c_int) as isize);
        let mut i_0: libc::c_int = *p_0.offset(0 as libc::c_int as isize);
        let mut j_0: libc::c_int = *p_0.offset(1 as libc::c_int as isize);
        let mut thispt: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
        let mut horizonpt: kmVec3 = kmVec3 { x: 0., y: 0., z: 0. };
        ...
        let mut stack_top: libc::c_int = 0 as libc::c_int;
        *convex_hull.offset(0 as libc::c_int as isize) = thispt;
        ...
        while i_0 >= 0 as libc::c_int && i_0 < w && j_0 >= 0 as libc::c_int && j_0 < h {
            ...
            while stack_top > 0 as libc::c_int {
                let mut s1: libc::c_float = azimuth_slope(
                    thispt,
                    *convex_hull.offset(stack_top as isize),
                );
                let mut s2: libc::c_float = azimuth_slope(
                    thispt,
            ...
            *convex_hull.offset(stack_top as isize) = thispt;
            let mut occlusion: libc::c_float = compute_occlusion(thispt, horizonpt);
...
pub unsafe extern "C" fn heman_lighting_compute_occlusion(
    ...
    let mut startpts: *mut libc::c_int = malloc(
    ...
    while i < 16 as libc::c_int {
        let mut dx: libc::c_int = scans[(i * 2 as libc::c_int) as usize];
        let mut dy: libc::c_int = scans[(i * 2 as libc::c_int + 1 as libc::c_int)
            as usize];
