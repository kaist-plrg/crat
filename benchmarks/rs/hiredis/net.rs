...
unsafe extern "C" fn _redisContextConnectTcp(
    ...
    if (*c).tcp.host != addr as *mut libc::c_char {
        ...
        if ((*c).tcp.host).is_null() {
            current_block = 12441764094093708892;
        } else {
                        ...
                        's_170: loop {
                            ...
                            loop {
                                ...
                                if *__errno_location() == 113 as libc::c_int {
                                    ...
                                    if blocking != 0 {
                                        current_block = 10426247882940499550;
                                        break;
                                    } else {
                                        current_block = 16029476503615101993;
                                        break;
                            ...
                            rv = 0 as libc::c_int;
                            current_block = 5141974893753586580;
                            break;
                        }
                        match current_block {
                            13181116735258129520 => {}
                            12441764094093708892 => {}
                            5141974893753586580 => {}
                            _ => {
                                if p.is_null() {
...
pub unsafe extern "C" fn redisContextConnectUnix(
    ...
    if (*c).unix_sock.path != path as *mut libc::c_char {
        ...
        if ((*c).unix_sock.path).is_null() {
            current_block = 13675112013682232052;
        } else {
