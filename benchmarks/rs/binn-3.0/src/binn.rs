...
pub unsafe extern "C" fn binn_malloc(mut size: libc::c_int) -> *mut libc::c_void {
    ...
    return malloc_fn.unwrap()(size as size_t);
}
...
pub unsafe extern "C" fn binn_get_type_info(
    ...
    loop {
        if long_type < 0 as libc::c_int {
            current_block_11 = 9795995725123450840;
            break;
        }
        ...
        224 | 225 | 226 => {
            if !(item.is_null() || size < 0 as libc::c_int) {
                if size < 3 as libc::c_int {
                    if !pointer.is_null() {
                        current_block = 12864548124352848941;
                    } else {
...
pub unsafe extern "C" fn binn_new(
    ...
    if binn_create(item, type_0, size, pointer) == 0 as libc::c_int {
        free_fn.unwrap()(item as *mut libc::c_void);
...
pub unsafe extern "C" fn CheckAllocation(
    ...
    if (*item).used_size + add_size > (*item).alloc_size {
        ...
        ptr = realloc_fn.unwrap()((*item).pbuf, alloc_size as size_t);
        if ptr.is_null() {
            return 0 as libc::c_int;
        }
        (*item).pbuf = ptr;
...
pub unsafe extern "C" fn compress_int(
    ...
    match type_0 {
        129 => {
            ...
            current_block = 17506093557660523453;
        }
        97 => {
        ...
        128 => {
            ...
            current_block = 17974108756977424592;
        }
        96 => {
    ...
    match current_block {
        17506093557660523453 => {
            if vint >= 0 as libc::c_int as libc::c_longlong {
                ...
                current_block = 14623761261893752754;
            }
        }
        _ => {}
    }
    match current_block {
        17974108756977424592 => {
...
pub unsafe extern "C" fn AddValue(
    ...
    if pvalue.is_null() {
        ...
        match storage_type {
            ...
            192 | 160 => {
                if size == 0 as libc::c_int {
                    ...
                    current_block_1 = 1406606467991079928;
                }
            }
            _ => {
                current_block_1 = 1406606467991079928;
        ...
        match current_block_1 {
            1406606467991079928 => return 0 as libc::c_int,
            _ => {}
        }
    }
...
pub unsafe extern "C" fn binn_free(mut item: *mut binn) {
    ...
    if (*item).writable != 0 && (*item).pre_allocated == 0 as libc::c_int {
        free_fn.unwrap()((*item).pbuf);
    }
    if ((*item).freefn).is_some() {
        ((*item).freefn).unwrap()((*item).ptr);
    }
    if (*item).allocated != 0 {
        free_fn.unwrap()(item as *mut libc::c_void);
    } else {
...
pub unsafe extern "C" fn binn_is_valid_ex(
    ...
    loop {
        ...
        p = AdvanceDataPos(p, plimit);
        if p.is_null() || p < base {
            current_block = 15350889812971402410;
            break;
        }
        i += 1;
        i;
    ...
    match current_block {
        15350889812971402410 => return 0 as libc::c_int,
        _ => {
            if !ptype.is_null() && *ptype == 0 as libc::c_int {
                *ptype = type_0;
            }
...
pub unsafe extern "C" fn binn_read_pair(
    ...
    loop {
        ...
        counter;
        if counter == pos {
            current_block = 4210294864639715551;
            break;
        }
        p = AdvanceDataPos(p, plimit);
        if p.is_null() || p < base {
...
pub unsafe extern "C" fn binn_map_pair(
    ...
    if binn_read_pair(0xe1 as libc::c_int, map, pos, pid, 0 as *mut libc::c_char, value)
        == 0 as libc::c_int
    {
        free_fn.unwrap()(value as *mut libc::c_void);
        return 0 as *mut binn;
    }
...
pub unsafe extern "C" fn GetWriteConvertedData(
    ...
    if (*ppvalue).is_null() {
        ...
        match type_0 {
            ...
            160 | 192 => {
                if *psize == 0 as libc::c_int {
                    ...
                    current_block_4 = 2037384573717028209;
                }
            }
            _ => {
                current_block_4 = 2037384573717028209;
        ...
        match current_block_4 {
            2037384573717028209 => return 0 as libc::c_int,
            _ => {}
        }
    }
    match type_0 {
    ...
    if !item.is_null() {
        ...
        match storage_type {
            ...
            160 => {
                ...
                current_block_19 = 7020098407919514347;
            }
            192 | 224 => {
                current_block_19 = 7020098407919514347;
            }
    ...
    if type_family((*value).type_0) == 0xf2 as libc::c_int {
        ...
        match (*value).type_0 {
            98 => {
                ...
                current_block = 3952226170978762994;
            }
            130 => {
                current_block = 3952226170978762994;
            }
            ...
            524385 => {
                ...
                current_block = 5338893022254234028;
            }
