...
unsafe extern "C" fn snacc_size(n: size_t) -> size_t {
    let mut size: size_t = 0 as libc::c_int as size_t;
    size = (size as libc::c_ulong)
        .wrapping_add(::std::mem::size_of::<snacc_state_t>() as libc::c_ulong) as size_t
        as size_t;
    size = (size as libc::c_ulong)
        .wrapping_add(
            (2 as libc::c_int as libc::c_ulong)
                .wrapping_mul(n)
                .wrapping_mul(::std::mem::size_of::<snacc_type_t>() as libc::c_ulong),
        ) as size_t as size_t;
...
unsafe extern "C" fn snacc_init(
    ...
    (*state)
        ...
        .offset(::std::mem::size_of::<snacc_state_t>() as libc::c_ulong as isize)
        as *mut snacc_type_t;
    (*state)
        .work = ((*state).window as *mut libc::c_uchar)
        .offset(
            n.wrapping_mul(::std::mem::size_of::<snacc_type_t>() as libc::c_ulong)
                as isize,
        ) as *mut snacc_type_t;
    (*state)
