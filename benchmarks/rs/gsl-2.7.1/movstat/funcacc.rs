...
unsafe extern "C" fn funcacc_size(n: size_t) -> size_t {
    let mut size: size_t = 0 as libc::c_int as size_t;
    size = (size as libc::c_ulong)
        .wrapping_add(::std::mem::size_of::<funcacc_state_t>() as libc::c_ulong)
        as size_t as size_t;
    size = (size as libc::c_ulong)
        .wrapping_add(
            n.wrapping_mul(::std::mem::size_of::<funcacc_type_t>() as libc::c_ulong),
        ) as size_t as size_t;
...
unsafe extern "C" fn funcacc_init(
    ...
    (*state)
        ...
        .offset(::std::mem::size_of::<funcacc_state_t>() as libc::c_ulong as isize)
...
unsafe extern "C" fn funcacc_get(
    ...
    *result = (Some(((*f).function).unwrap())).unwrap()(n, (*state).window, (*f).params);
    return GSL_SUCCESS as libc::c_int;
}
