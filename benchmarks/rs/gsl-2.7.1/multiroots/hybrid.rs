...
unsafe extern "C" fn hybrid_set_impl(
    ...
    let mut status: libc::c_int = 0;
    status = (Some(((*func).f).unwrap())).unwrap()(x, (*func).params, f);
    if status != 0 {
        return status;
    }
...
unsafe extern "C" fn hybrid_iterate_impl(
    ...
    let mut status: libc::c_int = (Some(((*func).f).unwrap()))
        .unwrap()(x_trial, (*func).params, f_trial);
...
static mut hybrid_type: gsl_multiroot_fsolver_type = {
    let mut init = gsl_multiroot_fsolver_type {
        ...
        size: ::std::mem::size_of::<hybrid_state_t>() as libc::c_ulong,
        alloc: Some(
            hybrid_alloc
