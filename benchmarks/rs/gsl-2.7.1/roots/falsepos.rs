...
unsafe extern "C" fn falsepos_init(
    ...
    *root = 0.5f64 * (x_lower + x_upper);
    f_lower = (Some(((*f).function).unwrap())).unwrap()(x_lower, (*f).params);
    ...
    f_upper = (Some(((*f).function).unwrap())).unwrap()(x_upper, (*f).params);
...
unsafe extern "C" fn falsepos_iterate(
    ...
    f_linear = (Some(((*f).function).unwrap())).unwrap()(x_linear, (*f).params);
    ...
    f_bisect = (Some(((*f).function).unwrap())).unwrap()(x_bisect, (*f).params);
...
static mut falsepos_type: gsl_root_fsolver_type = {
    let mut init = gsl_root_fsolver_type {
        ...
        size: ::std::mem::size_of::<falsepos_state_t>() as libc::c_ulong,
        set: Some(
            falsepos_init
