...
unsafe extern "C" fn steffenson_init(
    ...
    (*state).f = (Some(((*fdf).f).unwrap())).unwrap()(x, (*fdf).params);
    (*state).df = (Some(((*fdf).df).unwrap())).unwrap()(x, (*fdf).params);
    (*state).x = x;
    (*state).x_1 = 0.0f64;
    (*state).x_2 = 0.0f64;
...
unsafe extern "C" fn steffenson_iterate(
    ...
    (Some(((*fdf).fdf).unwrap()))
        .unwrap()(x_new, (*fdf).params, &mut f_new, &mut df_new);
    (*state).x_2 = x_1;
    (*state).x_1 = x;
    (*state).x = x_new;
    (*state).f = f_new;
...
static mut steffenson_type: gsl_root_fdfsolver_type = {
    let mut init = gsl_root_fdfsolver_type {
        ...
        size: ::std::mem::size_of::<steffenson_state_t>() as libc::c_ulong,
        set: Some(
            steffenson_init
