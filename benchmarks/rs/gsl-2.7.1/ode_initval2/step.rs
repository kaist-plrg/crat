...
pub unsafe extern "C" fn gsl_odeiv2_step_alloc(
    ...
    let mut s: *mut gsl_odeiv2_step = malloc(
        ::std::mem::size_of::<gsl_odeiv2_step>() as libc::c_ulong,
    ...
    (*s).type_0 = T;
    (*s).dimension = dim;
    (*s).state = ((*(*s).type_0).alloc).unwrap()(dim);
    if ((*s).state).is_null() {
...
pub unsafe extern "C" fn gsl_odeiv2_step_order(
    ...
    return ((*(*s).type_0).order).unwrap()((*s).state);
}
pub unsafe extern "C" fn gsl_odeiv2_step_apply(
    ...
    return ((*(*s).type_0).apply)
        .unwrap()((*s).state, (*s).dimension, t, h, y, yerr, dydt_in, dydt_out, dydt);
}
pub unsafe extern "C" fn gsl_odeiv2_step_reset(
    mut s: *mut gsl_odeiv2_step,
) -> libc::c_int {
    return ((*(*s).type_0).reset).unwrap()((*s).state, (*s).dimension);
}
...
pub unsafe extern "C" fn gsl_odeiv2_step_set_driver(
    ...
    if !d.is_null() {
        ((*(*s).type_0).set_driver).unwrap()((*s).state, d);
    } else {
        gsl_error(
