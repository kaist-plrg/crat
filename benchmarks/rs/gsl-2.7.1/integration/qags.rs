...
unsafe extern "C" fn i_transform(
    ...
    let mut y: libc::c_double = (Some(((*f).function).unwrap())).unwrap()(x, (*f).params)
        + (Some(((*f).function).unwrap())).unwrap()(-x, (*f).params);
    return y / t / t;
}
...
unsafe extern "C" fn il_transform(
    ...
    let mut y: libc::c_double = (Some(((*f).function).unwrap()))
        .unwrap()(x, (*f).params);
    return y / t / t;
}
...
unsafe extern "C" fn qags(
    ...
    *abserr = 0 as libc::c_int as libc::c_double;
    ...
    q.unwrap()(f, a, b, &mut result0, &mut abserr0, &mut resabs0, &mut resasc0);
    ...
    loop {
        ...
        iteration;
        q.unwrap()(f, a1, b1, &mut area1, &mut error1, &mut resabs1, &mut resasc1);
        q.unwrap()(f, a2, b2, &mut area2, &mut error2, &mut resabs2, &mut resasc2);
        area12 = area1 + area2;
        ...
        if errsum <= tolerance {
            current_block = 763033763797464511;
            break;
        }
        if error_type != 0 {
            ...
            if err_ext == 1.7976931348623157e+308f64 {
                ...
                if error_type != 0 || error_type2 != 0 {
                    ...
                    if res_ext != 0.0f64 && area != 0.0f64 {
                        ...
                        current_block = 9562375429869172142;
                    } else {
                        current_block = 18002345992382212654;
                    }
                } else {
