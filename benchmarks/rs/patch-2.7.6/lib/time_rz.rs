...
unsafe extern "C" fn equal_tm(mut a: *const tm, mut b: *const tm) -> libc::c_int {
    return ((*a).tm_sec ^ (*b).tm_sec | (*a).tm_min ^ (*b).tm_min
        | (*a).tm_hour ^ (*b).tm_hour | (*a).tm_mday ^ (*b).tm_mday
        | (*a).tm_mon ^ (*b).tm_mon | (*a).tm_year ^ (*b).tm_year
        | isdst_differ((*a).tm_isdst, (*b).tm_isdst) as libc::c_int == 0) as libc::c_int;
}
...
pub unsafe extern "C" fn tzalloc(mut name: *const libc::c_char) -> timezone_t {
    ...
    let mut tz: timezone_t = malloc(
        (9 as libc::c_ulong)
            .wrapping_add(::std::mem::align_of::<tm_zone>() as libc::c_ulong)
            .wrapping_sub(1 as libc::c_int as libc::c_ulong)
            .wrapping_add(abbr_size)
            & !(::std::mem::align_of::<tm_zone>() as libc::c_ulong)
                .wrapping_sub(1 as libc::c_int as libc::c_ulong),
    ) as timezone_t;
    if !tz.is_null() {
...
unsafe extern "C" fn save_abbr(mut tz: timezone_t, mut tm: *mut tm) -> bool {
    ...
    if *zone != 0 {
        ...
        while strcmp(zone_copy, zone) != 0 as libc::c_int {
            ...
            {
                let mut zone_size: size_t = (strlen(zone))
                    .wrapping_add(1 as libc::c_int as libc::c_ulong);
                let mut zone_used: size_t = zone_copy
                    .offset_from(((*tz).abbrs).as_mut_ptr()) as libc::c_long as size_t;
                if (-(1 as libc::c_int) as size_t).wrapping_sub(zone_used) < zone_size {
                    *__errno_location() = 12 as libc::c_int;
                    return 0 as libc::c_int != 0;
                }
                if zone_used.wrapping_add(zone_size)
                    < ABBR_SIZE_MIN as libc::c_int as libc::c_ulong
                {
...
pub unsafe extern "C" fn mktime_z(mut tz: timezone_t, mut tm: *mut tm) -> time_t {
    if tz.is_null() {
        ...
        let mut old_tz: timezone_t = set_tz(tz);
        if !old_tz.is_null() {
            let mut t: time_t = mktime(tm);
            let mut badtime: time_t = -(1 as libc::c_int) as time_t;
            let mut tm_1: tm = tm {
                tm_sec: 0,
                tm_min: 0,
                tm_hour: 0,
            ...
            if (t != badtime
                || !(localtime_r(&mut t, &mut tm_1)).is_null()
                    && equal_tm(tm, &mut tm_1) != 0) && !save_abbr(tz, tm)
            {
                t = badtime;
            }
            if revert_tz(old_tz) {
