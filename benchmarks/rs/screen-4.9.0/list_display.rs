...
extern "C" {
    ...
    fn leftline(_: *mut libc::c_char, _: libc::c_int, _: *mut mchar);
...
static mut ListID: [libc::c_char; 8] = unsafe {
    *::std::mem::transmute::<&[u8; 8], &mut [libc::c_char; 8]>(b"display\0")
};
unsafe extern "C" fn gl_Display_header(mut ldata: *mut ListData) -> libc::c_int {
    leftline(
        b"term-type   size         user interface           window       Perms\0"
...
unsafe extern "C" fn gl_Display_footer(mut ldata: *mut ListData) -> libc::c_int {
    centerline(
        b"[Press Space to refresh; Return to end.]\0" as *const u8 as *const libc::c_char
            as *mut libc::c_char,
        (*flayer).l_height - 1 as libc::c_int,
    );
    return 1 as libc::c_int;
}
unsafe extern "C" fn gl_Display_row(
    mut ldata: *mut ListData,
    mut lrow: *mut ListRow,
) -> libc::c_int {
    let mut d: *mut display = (*lrow).data as *mut display;
    let mut tbuf: [libc::c_char; 80] = [0; 80];
    static mut blockstates: [*mut libc::c_char; 5] = [
        b"nb\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
        b"NB\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
        b"Z<\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
        b"Z>\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
        b"BL\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
    ];
    let mut w: *mut win = (*d).d_fore;
    let mut m_current: mchar = mchar_blank;
    m_current.attr = ((1 as libc::c_int) << 2 as libc::c_int) as libc::c_uchar;
    sprintf(
        tbuf.as_mut_ptr(),
        b" %-10.10s%4dx%-4d%10.10s@%-16.16s%s\0" as *const u8 as *const libc::c_char,
        ((*d).d_termname).as_mut_ptr(),
        (*d).d_width,
        (*d).d_height,
        ((*(*d).d_user).u_name).as_mut_ptr(),
        ((*d).d_usertty).as_mut_ptr(),
        if ((*d).d_blocked != 0 || (*d).d_nonblock >= 0 as libc::c_int)
            && (*d).d_blocked <= 4 as libc::c_int
        {
            blockstates[(*d).d_blocked as usize] as *const libc::c_char
        } else {
            b"  \0" as *const u8 as *const libc::c_char
        },
    );
    if !w.is_null() {
        let mut l: libc::c_int = (10 as libc::c_int as libc::c_ulong)
            .wrapping_sub(strlen((*w).w_title)) as libc::c_int;
        if l < 0 as libc::c_int {
            l = 0 as libc::c_int;
        }
        sprintf(
            tbuf.as_mut_ptr().offset(strlen(tbuf.as_mut_ptr()) as isize),
            b"%3d(%.10s)%*s%c%c%c%c\0" as *const u8 as *const libc::c_char,
            (*w).w_number,
            (*w).w_title,
            l,
            b"\0" as *const u8 as *const libc::c_char,
            if 0 as libc::c_int != 0 { '&' as i32 } else { ' ' as i32 },
            if AclCheckPermWin((*d).d_user, 2 as libc::c_int, w) != 0 {
                '-' as i32
            } else if (*w).w_wlock == 0 as libc::c_int || (*d).d_user == (*w).w_wlockuser
            {
                'r' as i32
            } else {
                'R' as i32
            ...
            if AclCheckPermWin((*d).d_user, 2 as libc::c_int, w) != 0 {
                '-' as i32
            } else if (*w).w_wlock == 0 as libc::c_int {
                'w' as i32
            } else if (*d).d_user == (*w).w_wlockuser {
                'W' as i32
            } else {
                'v' as i32
            },
...
unsafe extern "C" fn gl_Display_rebuild(mut ldata: *mut ListData) {
    let mut d: *mut display = 0 as *mut display;
    let mut row: *mut ListRow = 0 as *mut ListRow;
    d = displays;
    while !d.is_null() {
        row = glist_add_row(ldata, d as *mut libc::c_void, row);
        if d == display {
            (*ldata).selected = row;
        }
        d = (*d).d_next;
...
unsafe extern "C" fn gl_Display_input(
    mut ldata: *mut ListData,
    ...
    *len;
    match ch as libc::c_int {
        ...
        100 | 68 => {
            display = (*(*ldata).selected).data as *mut display;
            if !(display == cd) {
                Detach(
...
unsafe extern "C" fn gl_Display_freerow(
    mut ldata: *mut ListData,
    mut row: *mut ListRow,
) -> libc::c_int {
    return 0 as libc::c_int;
}
unsafe extern "C" fn gl_Display_free(mut ldata: *mut ListData) -> libc::c_int {
    return 0 as libc::c_int;
}
static mut gl_Display: GenericList = unsafe {
    {
        let mut init = GenericList {
            gl_printheader: Some(
                gl_Display_header as unsafe extern "C" fn(*mut ListData) -> libc::c_int,
            ),
            gl_printfooter: Some(
                gl_Display_footer as unsafe extern "C" fn(*mut ListData) -> libc::c_int,
            ),
            gl_printrow: Some(
                gl_Display_row
            ...
            gl_free: Some(
                gl_Display_free as unsafe extern "C" fn(*mut ListData) -> libc::c_int,
            ),
            gl_matchrow: None,
        };
        init
...
pub unsafe extern "C" fn display_displays() {
    let mut ldata: *mut ListData = 0 as *mut ListData;
    if (*flayer).l_width < 10 as libc::c_int || (*flayer).l_height < 5 as libc::c_int {
        LMsg(
            0 as libc::c_int,
            b"Window size too small for displays page\0" as *const u8
                as *const libc::c_char,
        );
        return;
    }
    ldata = glist_display(&mut gl_Display, ListID.as_mut_ptr());
    if ldata.is_null() {
        return;
    }
    gl_Display_rebuild(ldata);
