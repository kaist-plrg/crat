...
unsafe extern "C" fn alloc_string_sized(
    ...
    let mut out: *mut libc::c_char = lodepng_malloc(
        insize.wrapping_add(1 as libc::c_int as libc::c_ulong),
    ) as *mut libc::c_char;
    if !out.is_null() {
...
unsafe extern "C" fn advanceBits(mut reader: *mut LodePNGBitReader, mut nbits: size_t) {
    (*reader).buffer >>= nbits;
    (*reader)
        .bp = ((*reader).bp as libc::c_ulong).wrapping_add(nbits) as size_t as size_t;
}
...
unsafe extern "C" fn bpmnode_sort(mut leaves: *mut BPMNode, mut num: size_t) {
    ...
    while width < num {
        let mut a: *mut BPMNode = if counter & 1 as libc::c_int as libc::c_ulong != 0 {
            mem
        } else {
            leaves
        };
        let mut b: *mut BPMNode = if counter & 1 as libc::c_int as libc::c_ulong != 0 {
            leaves
        } else {
            mem
        };
            ...
            && code_ll <= 285 as libc::c_int as libc::c_uint
        {
            ...
            if code_d > 29 as libc::c_int as libc::c_uint {
                ...
                if distance as libc::c_ulong > start {
                    ...
                    (*out)
                        .size = ((*out).size as libc::c_ulong).wrapping_add(length)
                        as size_t as size_t;
...
unsafe extern "C" fn inflatev(
    ...
    if ((*settings).custom_inflate).is_some() {
        let mut error: libc::c_uint = ((*settings).custom_inflate)
            .unwrap()(&mut (*out).data, &mut (*out).size, in_0, insize, settings);
...
unsafe extern "C" fn searchCodeIndex(
    ...
    let mut right: size_t = array_size.wrapping_sub(1 as libc::c_int as libc::c_ulong);
    while left <= right {
...
unsafe extern "C" fn deflate(
    ...
    if ((*settings).custom_deflate).is_some() {
        let mut error: libc::c_uint = ((*settings).custom_deflate)
            .unwrap()(out, outsize, in_0, insize, settings);
...
unsafe extern "C" fn zlib_decompress(
    ...
    if ((*settings).custom_zlib).is_some() {
        error = ((*settings).custom_zlib).unwrap()(out, outsize, in_0, insize, settings);
        if error != 0 {
            error = 110 as libc::c_int as libc::c_uint;
...
unsafe extern "C" fn getPixelColorRGBA8(
        ...
        if (*mode).bitdepth == 8 as libc::c_int as libc::c_uint {
            ...
            *r = *g;
            ...
            let mut highest: libc::c_uint = ((1 as libc::c_uint) << (*mode).bitdepth)
                .wrapping_sub(1 as libc::c_uint);
            let mut j: size_t = i.wrapping_mul((*mode).bitdepth as libc::c_ulong);
        ...
        == LCT_PALETTE as libc::c_int as libc::c_uint
    {
        ...
        if (*mode).bitdepth == 8 as libc::c_int as libc::c_uint {
            ...
            let mut j_0: size_t = i.wrapping_mul((*mode).bitdepth as libc::c_ulong);
...
pub unsafe extern "C" fn lodepng_compute_color_stats(
    ...
    (*stats)
        .numpixels = ((*stats).numpixels as libc::c_ulong).wrapping_add(numpixels)
        as size_t as size_t;
    ...
    if numcolors_done == 0 {
        ...
        loop {
            ...
            if error != 0 {
                current_block = 13006983655005890919;
                break;
            }
            i = i.wrapping_add(1);
            i;
        ...
        && strategy as libc::c_uint <= LFS_FOUR as libc::c_int as libc::c_uint
    {
        ...
        while y != h {
            let mut outindex: size_t = (1 as libc::c_int as libc::c_ulong)
                .wrapping_add(linebytes)
                .wrapping_mul(y as libc::c_ulong);
            let mut inindex: size_t = linebytes.wrapping_mul(y as libc::c_ulong);
        ...
        while y != h {
            let mut outindex_0: size_t = (1 as libc::c_int as libc::c_ulong)
                .wrapping_add(linebytes)
                .wrapping_mul(y as libc::c_ulong);
            let mut inindex_0: size_t = linebytes.wrapping_mul(y as libc::c_ulong);
        ...
        if !((*state).error != 0) {
            ...
            if !((*state).error != 0) {
                ...
                if (*state).encoder.auto_convert != 0 {
                    ...
                    if (*state).error != 0 {
                        current_block = 2971598661881991089;
                    } else {
