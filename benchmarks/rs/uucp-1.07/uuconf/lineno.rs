...
pub static mut _uuconf_lineno_rcsid: [libc::c_char; 50] = unsafe {
    *::std::mem::transmute::<
        &[u8; 50],
        &[libc::c_char; 50],
    >(b"$Id: lineno.c,v 1.6 2002/03/05 19:10:42 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_error_lineno(mut pglobal: pointer) -> libc::c_int {
    let mut qglobal: *mut sglobal = pglobal as *mut sglobal;
    if qglobal.is_null() { return 0 as libc::c_int } else { return (*qglobal).ilineno };
}
