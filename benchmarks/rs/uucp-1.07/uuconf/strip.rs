...
pub static mut _uuconf_strip_rcsid: [libc::c_char; 49] = unsafe {
    *::std::mem::transmute::<
        &[u8; 49],
        &[libc::c_char; 49],
    >(b"$Id: strip.c,v 1.3 2002/03/05 19:10:42 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_strip(
    mut pglobal: pointer,
    mut pistrip: *mut libc::c_int,
) -> libc::c_int {
    let mut qglobal: *mut sglobal = pglobal as *mut sglobal;
    *pistrip = 0 as libc::c_int;
    if (*(*qglobal).qprocess).fstrip_login != 0 {
        *pistrip |= 0o1 as libc::c_int;
    }
    if (*(*qglobal).qprocess).fstrip_proto != 0 {
        *pistrip |= 0o2 as libc::c_int;
    }
