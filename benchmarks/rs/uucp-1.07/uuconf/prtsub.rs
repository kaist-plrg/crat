...
pub static mut _uuconf_prtsub_rcsid: [libc::c_char; 50] = unsafe {
    *::std::mem::transmute::<
        &[u8; 50],
        &[libc::c_char; 50],
    >(b"$Id: prtsub.c,v 1.8 2002/03/05 19:10:42 ian Rel $\0")
};
pub unsafe extern "C" fn _uuconf_uclear_port(mut qport: *mut uuconf_port) {
    (*qport).uuconf_zname = 0 as *mut libc::c_char;
    (*qport).uuconf_ttype = UUCONF_PORTTYPE_UNKNOWN;
    (*qport).uuconf_zprotocols = 0 as *mut libc::c_char;
    (*qport).uuconf_qproto_params = 0 as *mut uuconf_proto_param;
    (*qport)
        .uuconf_ireliable = 0o4 as libc::c_int | 0o2 as libc::c_int
        | 0o20 as libc::c_int;
    (*qport).uuconf_zlockname = 0 as *mut libc::c_char;
    (*qport).uuconf_palloc = 0 as *mut libc::c_void;
}
