...
pub static mut _uuconf_tval_rcsid: [libc::c_char; 48] = unsafe {
    *::std::mem::transmute::<
        &[u8; 48],
        &[libc::c_char; 48],
    >(b"$Id: tval.c,v 1.6 2002/03/05 19:10:43 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_taylor_validate(
    mut pglobal: pointer,
    mut qsys: *const uuconf_system,
    mut zlogin: *const libc::c_char,
) -> libc::c_int {
    let mut qglobal: *mut sglobal = pglobal as *mut sglobal;
    let mut q: *mut svalidate = 0 as *mut svalidate;
    ...
    while !q.is_null() {
        if strcmp((*q).zlogname, zlogin) == 0 as libc::c_int {
            let mut pz: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
            pz = (*q).pzmachines;
            while !(*pz).is_null() {
                if strcmp(*pz, (*qsys).uuconf_zname) == 0 as libc::c_int {
