...
pub static mut _uuconf_locnm_rcsid: [libc::c_char; 49] = unsafe {
    *::std::mem::transmute::<
        &[u8; 49],
        &[libc::c_char; 49],
    >(b"$Id: locnm.c,v 1.7 2002/03/05 19:10:42 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_localname(
    mut pglobal: pointer,
    mut pzname: *mut *const libc::c_char,
) -> libc::c_int {
    let mut qglobal: *mut sglobal = pglobal as *mut sglobal;
    *pzname = (*(*qglobal).qprocess).zlocalname;
    if !(*pzname).is_null() { return 0 as libc::c_int } else { return 1 as libc::c_int };
}
