...
pub static mut _uuconf_tsnams_rcsid: [libc::c_char; 50] = unsafe {
    *::std::mem::transmute::<
        &[u8; 50],
        &[libc::c_char; 50],
    >(b"$Id: tsnams.c,v 1.6 2002/03/05 19:10:43 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_taylor_system_names(
    mut pglobal: pointer,
    ...
    *ppzsystems = 0 as *mut *mut libc::c_char;
    ...
    while !q.is_null() {
        if !(falias == 0 && (*q).falias != 0) {
            iret = _uuconf_iadd_string(
                qglobal,
                (*q).zname as *mut libc::c_char,
                1 as libc::c_int,
                0 as libc::c_int,
                ppzsystems,
    ...
    pz = *ppzsystems;
    i = c / 2 as libc::c_int - 1 as libc::c_int;
    while i >= 0 as libc::c_int {
        let mut zhold: *mut libc::c_char = 0 as *mut libc::c_char;
        zhold = *pz.offset(i as isize);
        let ref mut fresh0 = *pz.offset(i as isize);
        *fresh0 = *pz.offset((c - i - 1 as libc::c_int) as isize);
        let ref mut fresh1 = *pz.offset((c - i - 1 as libc::c_int) as isize);
        *fresh1 = zhold;
        i -= 1;
        i;
    }
