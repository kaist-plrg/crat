...
pub static mut _uuconf_runuxq_rcsid: [libc::c_char; 50] = unsafe {
    *::std::mem::transmute::<
        &[u8; 50],
        &[libc::c_char; 50],
    >(b"$Id: runuxq.c,v 1.6 2002/03/05 19:10:42 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_runuuxqt(
    mut pglobal: pointer,
    mut pirunuuxqt: *mut libc::c_int,
) -> libc::c_int {
    let mut qglobal: *mut sglobal = pglobal as *mut sglobal;
    let mut zrun: *const libc::c_char = 0 as *const libc::c_char;
    zrun = (*(*qglobal).qprocess).zrunuuxqt;
    if zrun.is_null()
        || strcasecmp(zrun, b"once\0" as *const u8 as *const libc::c_char)
            == 0 as libc::c_int
    {
        *pirunuuxqt = -(1 as libc::c_int);
    } else if strcasecmp(zrun, b"never\0" as *const u8 as *const libc::c_char)
        == 0 as libc::c_int
    {
        *pirunuuxqt = 0 as libc::c_int;
    } else if strcasecmp(zrun, b"percall\0" as *const u8 as *const libc::c_char)
        == 0 as libc::c_int
    {
        *pirunuuxqt = -(2 as libc::c_int);
    } else {
        let mut zend: *mut libc::c_char = 0 as *mut libc::c_char;
        *pirunuuxqt = strtol(
            (*(*qglobal).qprocess).zrunuuxqt as *mut libc::c_char,
            &mut zend,
            10 as libc::c_int,
        ) as libc::c_int;
        if *zend as libc::c_int != '\0' as i32 || *pirunuuxqt <= 0 as libc::c_int {
            *pirunuuxqt = -(1 as libc::c_int);
        }
    }
