...
pub static mut _uuconf_snams_rcsid: [libc::c_char; 49] = unsafe {
    *::std::mem::transmute::<
        &[u8; 49],
        &[libc::c_char; 49],
    >(b"$Id: snams.c,v 1.6 2002/03/05 19:10:42 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_system_names(
    mut pglobal: pointer,
    mut ppzsystems: *mut *mut *mut libc::c_char,
    mut falias: libc::c_int,
) -> libc::c_int {
    let mut qglobal: *mut sglobal = pglobal as *mut sglobal;
    let mut pztaylor: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
    let mut pzv2: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
    let mut pzhdb: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
    let mut iret: libc::c_int = 0;
    *ppzsystems = 0 as *mut *mut libc::c_char;
    pztaylor = 0 as *mut *mut libc::c_char;
    pzv2 = 0 as *mut *mut libc::c_char;
    pzhdb = 0 as *mut *mut libc::c_char;
    iret = uuconf_taylor_system_names(pglobal, &mut pztaylor, falias);
    if iret != 0 as libc::c_int {
        return iret;
    }
    if pzv2.is_null() && pzhdb.is_null() {
        *ppzsystems = pztaylor;
    } else if pztaylor.is_null() && pzhdb.is_null() {
        *ppzsystems = pzv2;
    } else if pztaylor.is_null() && pzv2.is_null() {
        *ppzsystems = pzhdb;
    } else {
        ...
        if !pzv2.is_null() && iret == 0 as libc::c_int {
            pz = pzv2;
            while !(*pz).is_null() {
                iret = _uuconf_iadd_string(
                    qglobal,
        ...
        if !pzhdb.is_null() && iret == 0 as libc::c_int {
            pz = pzhdb;
            while !(*pz).is_null() {
                iret = _uuconf_iadd_string(
                    qglobal,
        ...
        if !pzv2.is_null() {
            free(pzv2 as pointer);
        }
        if !pzhdb.is_null() {
            free(pzhdb as pointer);
        }
    }
    if iret == 0 as libc::c_int && (*ppzsystems).is_null() {
        iret = _uuconf_iadd_string(
            qglobal,
