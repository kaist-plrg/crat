...
pub static mut _uuconf_tlocnm_rcsid: [libc::c_char; 50] = unsafe {
    *::std::mem::transmute::<
        &[u8; 50],
        &[libc::c_char; 50],
    >(b"$Id: tlocnm.c,v 1.7 2002/03/05 19:10:43 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_taylor_login_localname(
    mut pglobal: pointer,
    ...
    if (*(*qglobal).qprocess).fuses_myname == 0 {
        *pzname = 0 as *mut libc::c_char;
        return 1 as libc::c_int;
    }
    iret = uuconf_taylor_system_names(pglobal, &mut pznames, 0 as libc::c_int);
    if iret != 0 as libc::c_int {
        return iret;
    }
    *pzname = 0 as *mut libc::c_char;
    ...
    while !(*pz).is_null() {
        ...
        iret = uuconf_system_info(pglobal, *pz, &mut ssys);
        if iret != 0 as libc::c_int {
            break;
        }
        qsys = &mut ssys;
        while !qsys.is_null() {
            if !((*qsys).uuconf_zlocalname).is_null() && (*qsys).uuconf_fcalled != 0
                && !((*qsys).uuconf_zcalled_login).is_null()
                && strcmp((*qsys).uuconf_zcalled_login, zlogin) == 0 as libc::c_int
            {
