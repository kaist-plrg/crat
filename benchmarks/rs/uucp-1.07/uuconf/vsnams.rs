...
pub static mut _uuconf_vsnams_rcsid: [libc::c_char; 51] = unsafe {
    *::std::mem::transmute::<
        &[u8; 51],
        &[libc::c_char; 51],
    >(b"$Id: vsnams.c,v 1.11 2002/03/05 19:10:43 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_v2_system_names(
    mut pglobal: pointer,
    ...
    *ppzsystems = 0 as *mut *mut libc::c_char;
    ...
    while _uuconf_getline(qglobal, &mut zline, &mut cline, e) > 0 as libc::c_int {
        ...
        *zname
        ...
        if *zname as libc::c_int == '\0' as i32 {
            continue;
        }
