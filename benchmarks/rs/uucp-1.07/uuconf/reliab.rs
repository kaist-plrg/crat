...
pub static mut _uuconf_reliab_rcsid: [libc::c_char; 50] = unsafe {
    *::std::mem::transmute::<
        &[u8; 50],
        &[libc::c_char; 50],
    >(b"$Id: reliab.c,v 1.9 2002/03/05 19:10:42 ian Rel $\0")
};
pub unsafe extern "C" fn _uuconf_iseven_bit(
    mut pglobal: pointer,
    mut argc: libc::c_int,
    ...
    iret = _uuconf_iboolean(qglobal, *argv.offset(1 as libc::c_int as isize), &mut fval);
    if iret & !(0x800 as libc::c_int) != 0 as libc::c_int {
        return iret;
    }
    *pi |= 0o1 as libc::c_int;
    if fval != 0 {
        *pi &= !(0o2 as libc::c_int);
    } else {
        *pi |= 0o2 as libc::c_int;
    }
    return iret;
}
pub unsafe extern "C" fn _uuconf_ireliable(
    mut pglobal: pointer,
    mut argc: libc::c_int,
    mut argv: *mut *mut libc::c_char,
    mut pvar: pointer,
    ...
    *pi |= 0o1 as libc::c_int;
    if fval != 0 {
        *pi |= 0o4 as libc::c_int;
    } else {
        *pi &= !(0o4 as libc::c_int);
    }
    return iret;
}
pub unsafe extern "C" fn _uuconf_ihalf_duplex(
    mut pglobal: pointer,
    mut argc: libc::c_int,
    mut argv: *mut *mut libc::c_char,
    mut pvar: pointer,
