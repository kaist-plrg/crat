...
pub static mut _uuconf_diacod_rcsid: [libc::c_char; 51] = unsafe {
    *::std::mem::transmute::<
        &[u8; 51],
        &[libc::c_char; 51],
    >(b"$Id: diacod.c,v 1.12 2002/03/05 19:10:42 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_dialcode(
    mut pglobal: pointer,
    mut zdial: *const libc::c_char,
    mut pznum: *mut *mut libc::c_char,
) -> libc::c_int {
    ...
    as_0[0 as libc::c_int as usize].uuconf_zcmd = zdial;
    as_0[0 as libc::c_int as usize]
        .uuconf_itype = 0x60 as libc::c_int | 0 as libc::c_int;
    as_0[0 as libc::c_int as usize].uuconf_pvar = pznum as pointer;
    as_0[0 as libc::c_int as usize]
        .uuconf_pifn = Some(
        idcode
    ...
    *pznum = 0 as *mut libc::c_char;
    ...
    while !(*pz).is_null() {
        ...
        if e.is_null() {
            ...
            fclose(e);
            if iret != 0 as libc::c_int || !(*pznum).is_null() {
                break;
            }
        }
        pz = pz.offset(1);
    ...
    } else if (*pznum).is_null() {
        iret = 1 as libc::c_int;
    }
    return iret;
}
unsafe extern "C" fn idcode(
    mut pglobal: pointer,
    mut argc: libc::c_int,
    ...
    let mut qglobal: *mut sglobal = pglobal as *mut sglobal;
    let mut pznum: *mut *mut libc::c_char = pvar as *mut *mut libc::c_char;
    if argc == 1 as libc::c_int {
        *pznum = malloc(1 as libc::c_int as libc::c_ulong) as *mut libc::c_char;
        if !(*pznum).is_null() {
            **pznum = '\0' as i32 as libc::c_char;
        }
    } else if argc == 2 as libc::c_int {
        *pznum = strdup(*argv.offset(1 as libc::c_int as isize));
    } else {
