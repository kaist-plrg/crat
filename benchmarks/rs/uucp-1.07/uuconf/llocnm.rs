...
extern "C" {
    ...
    fn uuconf_taylor_login_localname(
...
pub static mut _uuconf_llocnm_rcsid: [libc::c_char; 50] = unsafe {
    *::std::mem::transmute::<
        &[u8; 50],
        &[libc::c_char; 50],
    >(b"$Id: llocnm.c,v 1.9 2002/03/05 19:10:42 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_login_localname(
    mut pglobal: pointer,
    ...
    if !((*(*qglobal).qprocess).zlocalname).is_null() {
        *pzname = strdup((*(*qglobal).qprocess).zlocalname as *mut libc::c_char);
        if (*pzname).is_null() {
