...
>;
pub static mut _uuconf_tdial_rcsid: [libc::c_char; 49] = unsafe {
    *::std::mem::transmute::<
        &[u8; 49],
        &[libc::c_char; 49],
    >(b"$Id: tdial.c,v 1.9 2002/03/05 19:10:43 ian Rel $\0")
};
pub unsafe extern "C" fn uuconf_taylor_dialer_info(
    mut pglobal: pointer,
    ...
    while !(*pz).is_null() {
        ...
        let mut sdefault: uuconf_dialer = uuconf_dialer {
        ...
        if e.is_null() {
            ...
            as_0[0 as libc::c_int as usize]
                .uuconf_pvar = &mut zdialer as *mut *mut libc::c_char as pointer;
            as_0[0 as libc::c_int as usize]
                .uuconf_pifn = Some(
                iddialer
            ...
            if pblock.is_null() {
                ...
                _uuconf_uclear_dialer(&mut sdefault);
                sdefault.uuconf_palloc = pblock;
                zdialer = 0 as *mut libc::c_char;
                iret = uuconf_cmd_file(
                    pglobal,
                    e,
                    as_0.as_mut_ptr(),
                    &mut sdefault as *mut uuconf_dialer as pointer,
                    Some(
                        idunknown
                            as unsafe extern "C" fn(
                                pointer,
                ...
                while iret == 0 as libc::c_int && !zdialer.is_null()
                    && strcmp(zname, zdialer) != 0 as libc::c_int
                {
                    free(zdialer as pointer);
                    zdialer = 0 as *mut libc::c_char;
                ...
                if iret != 0 as libc::c_int {
                    ...
                    break;
                } else if !zdialer.is_null() {
                    let mut csize: size_t = 0;
                    as_0[0 as libc::c_int as usize].uuconf_pvar = 0 as *mut libc::c_void;
                    *qdialer = sdefault;
                    csize = (strlen(zdialer))
                        .wrapping_add(1 as libc::c_int as libc::c_ulong);
                    (*qdialer)
                        .uuconf_zname = uuconf_malloc(pblock, csize)
                        as *mut libc::c_char;
                    if ((*qdialer).uuconf_zname).is_null() {
                        (*qglobal).ierrno = *__errno_location();
                        free(zdialer as pointer);
                        iret = 4 as libc::c_int | 0x100 as libc::c_int;
                        break;
                        ...
                        memcpy(
                            (*qdialer).uuconf_zname as pointer,
                            zdialer as pointer as *const libc::c_void,
                            csize,
                        );
                        free(zdialer as pointer);
                        ilineno = (*qglobal).ilineno;
