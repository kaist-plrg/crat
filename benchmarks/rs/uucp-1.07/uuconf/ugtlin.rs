...
pub static mut _uuconf_ugtlin_rcsid: [libc::c_char; 50] = unsafe {
    *::std::mem::transmute::<
        &[u8; 50],
        &[libc::c_char; 50],
    >(b"$Id: ugtlin.c,v 1.9 2002/03/05 19:10:43 ian Rel $\0")
};
pub unsafe extern "C" fn _uuconf_getline(
    mut qglobal: *mut sglobal,
    mut pzline: *mut *mut libc::c_char,
    mut pcline: *mut size_t,
    mut e: *mut FILE,
) -> libc::c_int {
    let mut ctot: libc::c_int = 0;
    let mut zline: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut cline: size_t = 0;
    ctot = -(1 as libc::c_int);
    zline = 0 as *mut libc::c_char;
    cline = 0 as libc::c_int as size_t;
    loop {
        let mut cchars: libc::c_int = 0;
        if ctot < 0 as libc::c_int {
            cchars = getline(pzline, pcline, e) as libc::c_int;
        } else {
        ...
        if cchars < 0 as libc::c_int {
            ...
            if ctot >= 0 as libc::c_int { return ctot } else { return cchars }
        }
        if ctot < 0 as libc::c_int {
            ctot = cchars;
        } else {
            if *pcline <= (ctot + cchars) as size_t {
                let mut znew: *mut libc::c_char = 0 as *mut libc::c_char;
                if *pcline > 0 as libc::c_int as libc::c_ulong {
                    znew = realloc(
                        *pzline as pointer,
                        (ctot + cchars + 1 as libc::c_int) as size_t,
                    ) as *mut libc::c_char;
                } else {
                    znew = malloc((ctot + cchars + 1 as libc::c_int) as size_t)
                        as *mut libc::c_char;
                }
                if znew.is_null() {
                    free(zline as pointer);
                ...
                *pzline = znew;
                *pcline = (ctot + cchars + 1 as libc::c_int) as size_t;
            }
            memcpy(
                (*pzline).offset(ctot as isize) as pointer,
                zline as pointer as *const libc::c_void,
                (cchars + 1 as libc::c_int) as size_t,
            );
            ctot += cchars;
        }
        if ctot < 2 as libc::c_int
            || *(*pzline).offset((ctot - 1 as libc::c_int) as isize) as libc::c_int
                != '\n' as i32
            || *(*pzline).offset((ctot - 2 as libc::c_int) as isize) as libc::c_int
                != '\\' as i32
        {
            if !zline.is_null() {
                free(zline as pointer);
        ...
        (*qglobal).ilineno += 1;
        (*qglobal).ilineno;
        ctot -= 2 as libc::c_int;
        *(*pzline).offset(ctot as isize) = '\0' as i32 as libc::c_char;
    };
}
