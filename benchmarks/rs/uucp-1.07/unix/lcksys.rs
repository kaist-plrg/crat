...
unsafe extern "C" fn fslock_sys(
    ...
    clen = strlen(zname);
    z = zbufalc(
        (::std::mem::size_of::<[libc::c_char; 6]>() as libc::c_ulong).wrapping_add(clen),
    );
    memcpy(
...
pub unsafe extern "C" fn fsysdep_lock_system(mut qsys: *const uuconf_system) -> boolean {
    return fslock_sys(1 as libc::c_int, (*qsys).uuconf_zname);
}
pub unsafe extern "C" fn fsysdep_unlock_system(
    mut qsys: *const uuconf_system,
) -> boolean {
    return fslock_sys(0 as libc::c_int, (*qsys).uuconf_zname);
}
