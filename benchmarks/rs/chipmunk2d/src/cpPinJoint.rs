...
unsafe extern "C" fn preStep(mut joint: *mut cpPinJoint, mut dt: cpFloat) {
    ...
    (*joint)
        ...
        1.0f32 as libc::c_double
            / (if dist != 0. { dist } else { ::std::f32::INFINITY as cpFloat }),
    );
    (*joint)
        .nMass = 1.0f32 as libc::c_double
        / k_scalar(a, b, (*joint).r1, (*joint).r2, (*joint).n);
    let mut maxBias: cpFloat = (*joint).constraint.maxBias;
    (*joint)
        .bias = cpfclamp(
        -bias_coef((*joint).constraint.errorBias, dt) * (dist - (*joint).dist) / dt,
        -maxBias,
        maxBias,
    );
}
unsafe extern "C" fn applyCachedImpulse(
    ...
    let mut a: *mut cpBody = (*joint).constraint.a;
    let mut b: *mut cpBody = (*joint).constraint.b;
    let mut j: cpVect = cpvmult((*joint).n, (*joint).jnAcc * dt_coef);
    apply_impulses(a, b, (*joint).r1, (*joint).r2, j);
}
unsafe extern "C" fn applyImpulse(mut joint: *mut cpPinJoint, mut dt: cpFloat) {
    let mut a: *mut cpBody = (*joint).constraint.a;
    let mut b: *mut cpBody = (*joint).constraint.b;
    let mut n: cpVect = (*joint).n;
    let mut vrn: cpFloat = normal_relative_velocity(a, b, (*joint).r1, (*joint).r2, n);
    let mut jnMax: cpFloat = (*joint).constraint.maxForce * dt;
    let mut jn: cpFloat = ((*joint).bias - vrn) * (*joint).nMass;
    let mut jnOld: cpFloat = (*joint).jnAcc;
    (*joint).jnAcc = cpfclamp(jnOld + jn, -jnMax, jnMax);
    jn = (*joint).jnAcc - jnOld;
    apply_impulses(a, b, (*joint).r1, (*joint).r2, cpvmult(n, jn));
}
unsafe extern "C" fn getImpulse(mut joint: *mut cpPinJoint) -> cpFloat {
...
static mut klass: cpConstraintClass = unsafe {
    {
        let mut init = cpConstraintClass {
            ...
            >(Some(preStep as unsafe extern "C" fn(*mut cpPinJoint, cpFloat) -> ())),
            ...
            >(
                Some(
                    applyImpulse as unsafe extern "C" fn(*mut cpPinJoint, cpFloat) -> (),
                ),
            ),
            getImpulse: ::std::mem::transmute::<
                Option::<unsafe extern "C" fn(*mut cpPinJoint) -> cpFloat>,
                cpConstraintGetImpulseImpl,
            >(Some(getImpulse as unsafe extern "C" fn(*mut cpPinJoint) -> cpFloat)),
        };
        init
    }
};
...
pub unsafe extern "C" fn cpPinJointInit(
    ...
    let mut p1: cpVect = if !a.is_null() {
        cpTransformPoint((*a).transform, anchorA)
    } else {
        anchorA
    };
    let mut p2: cpVect = if !b.is_null() {
        cpTransformPoint((*b).transform, anchorB)
    } else {
        anchorB
    };
...
pub unsafe extern "C" fn cpPinJointNew(
    ...
    return cpPinJointInit(cpPinJointAlloc(), a, b, anchorA, anchorB)
        as *mut cpConstraint;
}
