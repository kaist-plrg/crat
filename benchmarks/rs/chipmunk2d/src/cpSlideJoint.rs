...
unsafe extern "C" fn preStep(mut joint: *mut cpSlideJoint, mut dt: cpFloat) {
...
unsafe extern "C" fn applyImpulse(mut joint: *mut cpSlideJoint, mut dt: cpFloat) {
    if cpveql((*joint).n, cpvzero) != 0 {
        return;
    }
    let mut a: *mut cpBody = (*joint).constraint.a;
    let mut b: *mut cpBody = (*joint).constraint.b;
...
unsafe extern "C" fn getImpulse(mut joint: *mut cpConstraint) -> cpFloat {
    return cpfabs((*(joint as *mut cpSlideJoint)).jnAcc);
}
static mut klass: cpConstraintClass = unsafe {
    {
        let mut init = cpConstraintClass {
            ...
            >(Some(preStep as unsafe extern "C" fn(*mut cpSlideJoint, cpFloat) -> ())),
...
pub unsafe extern "C" fn cpSlideJointNew(
    ...
    return cpSlideJointInit(cpSlideJointAlloc(), a, b, anchorA, anchorB, min, max)
        as *mut cpConstraint;
}
...
pub unsafe extern "C" fn cpSlideJointGetAnchorA(
    ...
    if cpConstraintIsSlideJoint(constraint) == 0 {
        cpMessage(
