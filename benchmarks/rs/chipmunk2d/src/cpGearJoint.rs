...
unsafe extern "C" fn applyImpulse(mut joint: *mut cpGearJoint, mut dt: cpFloat) {
    let mut a: *mut cpBody = (*joint).constraint.a;
    let mut b: *mut cpBody = (*joint).constraint.b;
    let mut wr: cpFloat = (*b).w * (*joint).ratio - (*a).w;
...
unsafe extern "C" fn getImpulse(mut joint: *mut cpGearJoint) -> cpFloat {
...
static mut klass: cpConstraintClass = unsafe {
    {
        let mut init = cpConstraintClass {
            ...
            >(Some(preStep as unsafe extern "C" fn(*mut cpGearJoint, cpFloat) -> ())),
            ...
            >(
                Some(
                    applyImpulse as unsafe extern "C" fn(*mut cpGearJoint, cpFloat) -> (),
                ),
            ),
            getImpulse: ::std::mem::transmute::<
                Option::<unsafe extern "C" fn(*mut cpGearJoint) -> cpFloat>,
                cpConstraintGetImpulseImpl,
            >(Some(getImpulse as unsafe extern "C" fn(*mut cpGearJoint) -> cpFloat)),
        };
        init
    }
};
...
pub unsafe extern "C" fn cpGearJointInit(
    ...
    (*joint).phase = phase;
    (*joint).ratio = ratio;
    (*joint).ratio_inv = 1.0f32 as libc::c_double / ratio;
...
pub unsafe extern "C" fn cpGearJointNew(
    ...
    return cpGearJointInit(cpGearJointAlloc(), a, b, phase, ratio) as *mut cpConstraint;
}
...
pub unsafe extern "C" fn cpGearJointGetPhase(
    ...
    if cpConstraintIsGearJoint(constraint) == 0 {
        cpMessage(
...
pub unsafe extern "C" fn cpGearJointSetRatio(
    ...
    (*(constraint as *mut cpGearJoint)).ratio_inv = 1.0f32 as libc::c_double / ratio;
}
