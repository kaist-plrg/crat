...
pub const KEY_CONFIGFILE: C2RustUnnamed_5 = 2;
pub const KEY_COMPRESS: C2RustUnnamed_5 = 1;
pub const KEY_PORT: C2RustUnnamed_5 = 0;
pub type C2RustUnnamed_6 = libc::c_uint;
pub const IDMAP_FILE: C2RustUnnamed_6 = 2;
pub const IDMAP_USER: C2RustUnnamed_6 = 1;
pub const IDMAP_NONE: C2RustUnnamed_6 = 0;
pub type C2RustUnnamed_7 = libc::c_uint;
pub const NOMAP_ERROR: C2RustUnnamed_7 = 1;
pub const NOMAP_IGNORE: C2RustUnnamed_7 = 0;
#[inline]
...
unsafe extern "C" fn buf_init(mut buf: *mut buffer, mut size: size_t) {
    if size != 0 {
        (*buf).p = malloc(size) as *mut uint8_t;
        if ((*buf).p).is_null() {
            fprintf(
                stderr,
...
unsafe extern "C" fn buf_resize(mut buf: *mut buffer, mut len: size_t) {
    ...
    (*buf).p = realloc((*buf).p as *mut libc::c_void, (*buf).size) as *mut uint8_t;
    if ((*buf).p).is_null() {
        fprintf(
            stderr,
            b"sshfs: memory allocation failed\n\0" as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn buf_get_attrs(
    ...
    (*stbuf).st_mtim.tv_sec = mtime as __time_t;
...
unsafe extern "C" fn start_ssh(mut conn: *mut conn) -> libc::c_int {
    ...
    if pid == -(1 as libc::c_int) {
        ...
        {
            perror(
                b"failed to redirect input/output\0" as *const u8 as *const libc::c_char,
            );
            _exit(1 as libc::c_int);
        }
        if sshfs.verbose == 0 && sshfs.foreground == 0 && devnull != -(1 as libc::c_int)
        {
...
unsafe extern "C" fn sftp_send_iov(
    ...
    buf_to_iov(&mut buf, &mut *iovout.as_mut_ptr().offset(fresh1 as isize));
    i = 0 as libc::c_int as libc::c_uint;
    while (i as libc::c_ulong) < count {
        let fresh2 = nout;
        nout = nout.wrapping_add(1);
        iovout[fresh2 as usize] = *iov.offset(i as isize);
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn sshfs_access(
    ...
    if mask & 1 as libc::c_int != 0 {
        err = ((*sshfs.op).getattr).unwrap()(path, &mut stbuf, 0 as *mut fuse_file_info);
        if err == 0 {
            if stbuf.st_mode & 0o170000 as libc::c_int as libc::c_uint
...
unsafe extern "C" fn sshfs_mkdir(
    ...
    buf_free(&mut buf);
    if err == -(1 as libc::c_int) {
        if ((*sshfs.op).access).unwrap()(path, 4 as libc::c_int) == 0 as libc::c_int {
...
unsafe extern "C" fn random_string(mut str: *mut libc::c_char, mut length: libc::c_int) {
    ...
    while i < length {
        let fresh3 = str;
        str = str.offset(1);
        *fresh3 = ('0' as i32 + rand_r(&mut sshfs.randseed) % 10 as libc::c_int)
            as libc::c_char;
        i += 1;
        i;
    }
...
unsafe extern "C" fn sshfs_open_common(
    ...
    if sshfs.max_conns > 1 as libc::c_int {
        ...
        if ce.is_null() {
            ...
            (*ce).conn = get_conn(0 as *const sshfs_file, 0 as *const libc::c_char);
            g_hash_table_insert(
                sshfs.conntab,
        ...
        (*sf).conn = &mut *(sshfs.conns).offset(0 as libc::c_int as isize) as *mut conn;
    ...
    if (*chunk).sio.error != 0 {
        ...
        while list_empty(&mut (*chunk).reqs) == 0 && size != 0 {
            ...
            if (*rreq).res < 0 as libc::c_int as libc::c_long {
                ...
                (*rreq)
                    .res = ((*rreq).res as libc::c_ulong).wrapping_sub(size) as ssize_t
                    as ssize_t;
                (*rreq)
        ...
        if res > 0 as libc::c_int {
            (*chunk).offset += res as libc::c_long;
            (*chunk)
                .size = ((*chunk).size as libc::c_ulong)
                .wrapping_sub(res as libc::c_ulong) as size_t as size_t;
        }
    }
...
unsafe extern "C" fn sshfs_statfs(
    ...
    (*buf)
        .f_bavail = (1000 as libc::c_ulonglong)
    ...
    (*buf).f_ffree = 1000000000 as libc::c_int as __fsfilcnt64_t;
    ...
    match key {
        ...
        -2 => {
            if (sshfs.host).is_null() && !(strchr(arg, ':' as i32)).is_null() {
                ...
                return 0 as libc::c_int;
            } else if (sshfs.mountpoint).is_null() {
...
unsafe extern "C" fn read_id_map(
    ...
    *r_idmap = g_hash_table_new(None, None);
    ...
    while !(fgets(line.as_mut_ptr(), 2048 as libc::c_int, fp)).is_null() {
        ...
        let mut local_id: *mut uint32_t = map_fn.unwrap()(name);
        if local_id.is_null() {
            if sshfs.debug != 0 {
                fprintf(
                    stderr,
