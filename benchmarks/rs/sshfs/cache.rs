...
unsafe extern "C" fn cache_init(
    ...
    res = ((*cache.next_oper).init).unwrap()(conn, cfg);
...
unsafe extern "C" fn cache_getattr(
    ...
    if err != 0 {
        ...
        err = ((*cache.next_oper).getattr).unwrap()(path, stbuf, fi);
        if err == 0 {
...
unsafe extern "C" fn cache_readlink(
    ...
    pthread_mutex_unlock(&mut cache.lock);
    err = ((*cache.next_oper).readlink).unwrap()(path, buf, size);
    if err == 0 {
...
unsafe extern "C" fn cache_releasedir(
    ...
    if (*cfi).is_open != 0 {
        (*fi).fh = (*cfi).fs_fh;
        err = ((*cache.next_oper).releasedir).unwrap()(path, fi);
    } else {
        err = 0 as libc::c_int;
    }
...
unsafe extern "C" fn cache_dirfill(
    ...
    err = ((*ch).filler).unwrap()((*ch).buf, name, stbuf, off, flags);
    if err == 0 {
...
unsafe extern "C" fn cache_readdir(
    ...
    if (*cfi).is_open != 0 {
        ...
        if ((*cache.next_oper).opendir).is_some() {
            err = ((*cache.next_oper).opendir).unwrap()(path, fi);
            if err != 0 {
                return err;
            }
        }
...
unsafe extern "C" fn cache_mknod(
    ...
    let mut err: libc::c_int = ((*cache.next_oper).mknod).unwrap()(path, mode, rdev);
    if err == 0 {
...
unsafe extern "C" fn cache_mkdir(
    ...
    let mut err: libc::c_int = ((*cache.next_oper).mkdir).unwrap()(path, mode);
    if err == 0 {
        cache_invalidate_dir(path);
    }
    return err;
...
unsafe extern "C" fn cache_unlink(mut path: *const libc::c_char) -> libc::c_int {
    let mut err: libc::c_int = ((*cache.next_oper).unlink).unwrap()(path);
    if err == 0 {
        cache_invalidate_dir(path);
    }
    return err;
...
unsafe extern "C" fn cache_rmdir(mut path: *const libc::c_char) -> libc::c_int {
    let mut err: libc::c_int = ((*cache.next_oper).rmdir).unwrap()(path);
    if err == 0 {
        cache_invalidate_dir(path);
    }
    return err;
...
unsafe extern "C" fn cache_symlink(
    ...
    let mut err: libc::c_int = ((*cache.next_oper).symlink).unwrap()(from, to);
    if err == 0 {
...
unsafe extern "C" fn cache_rename(
    ...
    let mut err: libc::c_int = ((*cache.next_oper).rename).unwrap()(from, to, flags);
    if err == 0 {
...
unsafe extern "C" fn cache_link(
    ...
    let mut err: libc::c_int = ((*cache.next_oper).link).unwrap()(from, to);
    if err == 0 {
...
unsafe extern "C" fn cache_chmod(
    ...
    let mut err: libc::c_int = ((*cache.next_oper).chmod).unwrap()(path, mode, fi);
    if err == 0 {
...
unsafe extern "C" fn cache_chown(
    ...
    let mut err: libc::c_int = ((*cache.next_oper).chown).unwrap()(path, uid, gid, fi);
    if err == 0 {
        cache_invalidate(path);
    }
    return err;
...
unsafe extern "C" fn cache_utimens(
    ...
    let mut err: libc::c_int = ((*cache.next_oper).utimens).unwrap()(path, tv, fi);
    if err == 0 {
        cache_invalidate(path);
    }
    return err;
...
unsafe extern "C" fn cache_write(
    ...
    let mut res: libc::c_int = ((*cache.next_oper).write)
        .unwrap()(path, buf, size, offset, fi);
...
unsafe extern "C" fn cache_create(
    ...
    let mut err: libc::c_int = ((*cache.next_oper).create).unwrap()(path, mode, fi);
    if err == 0 {
        cache_invalidate_dir(path);
    }
    return err;
...
unsafe extern "C" fn cache_truncate(
    ...
    let mut err: libc::c_int = ((*cache.next_oper).truncate).unwrap()(path, size, fi);
    if err == 0 {
        cache_invalidate(path);
    }
    return err;
...
unsafe extern "C" fn cache_fill(
    ...
    (*cache_oper)
        .getattr = if ((*oper).getattr).is_some() {
        Some(
            cache_getattr
                as unsafe extern "C" fn(
                    *const libc::c_char,
    ...
    (*cache_oper)
        .readdir = if ((*oper).readdir).is_some() {
        Some(
            cache_readdir
                as unsafe extern "C" fn(
                    *const libc::c_char,
    ...
    (*cache_oper)
        .readlink = if ((*oper).readlink).is_some() {
        Some(
            cache_readlink
                as unsafe extern "C" fn(
                    *const libc::c_char,
    ...
    (*cache_oper)
        .symlink = if ((*oper).symlink).is_some() {
        Some(
            cache_symlink
                as unsafe extern "C" fn(
                    *const libc::c_char,
    ...
    (*cache_oper)
        .truncate = if ((*oper).truncate).is_some() {
        Some(
            cache_truncate
                as unsafe extern "C" fn(
                    *const libc::c_char,
