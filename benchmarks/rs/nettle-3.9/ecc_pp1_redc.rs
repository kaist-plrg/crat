...
pub unsafe extern "C" fn _nettle_ecc_pp1_redc(
    ...
    if shift > 0 as libc::c_int as libc::c_uint {
        ...
        *rp
        ...
        cy = __gmpn_cnd_sub_n(hi, rp, rp as mp_srcptr, (*m).m, (*m).size as mp_size_t);
        if cy == hi {} else {
            __assert_fail(
                b"cy == hi\0" as *const u8 as *const libc::c_char,
                b"ecc-pp1-redc.c\0" as *const u8 as *const libc::c_char,
                67 as libc::c_int as libc::c_uint,
                (*::std::mem::transmute::<
                    &[u8; 79],
                    &[libc::c_char; 79],
                >(
                    b"void _nettle_ecc_pp1_redc(const struct ecc_modulo *, mp_limb_t *, mp_limb_t *)\0",
                ))
                    .as_ptr(),
            );
        }
