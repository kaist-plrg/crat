...
extern "C" {
    ...
    fn nettle_md5_compress(state: *mut uint32_t, data: *const uint8_t);
...
pub unsafe extern "C" fn nettle_md5_update(
    ...
    if (*ctx).index != 0 {
        let mut __md_left: libc::c_uint = (::std::mem::size_of::<[uint8_t; 64]>()
            as libc::c_ulong)
    ...
    match current_block_16 {
        5720623009719927633 => {
            while length >= ::std::mem::size_of::<[uint8_t; 64]>() as libc::c_ulong {
                nettle_md5_compress(((*ctx).state).as_mut_ptr(), data);
                (*ctx).count = ((*ctx).count).wrapping_add(1);
                (*ctx).count;
                data = data
                    .offset(
                        ::std::mem::size_of::<[uint8_t; 64]>() as libc::c_ulong as isize,
                    );
