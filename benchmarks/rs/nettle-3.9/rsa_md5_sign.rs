...
pub unsafe extern "C" fn nettle_rsa_md5_sign(
    mut key: *const rsa_private_key,
    mut hash: *mut md5_ctx,
    mut s: *mut __mpz_struct,
) -> libc::c_int {
    if nettle_pkcs1_rsa_md5_encode(s, (*key).size, hash) != 0 {
...
pub unsafe extern "C" fn nettle_rsa_md5_sign_digest(
    mut key: *const rsa_private_key,
    mut digest: *const uint8_t,
    mut s: *mut __mpz_struct,
) -> libc::c_int {
    if nettle_pkcs1_rsa_md5_encode_digest(s, (*key).size, digest) != 0 {
        nettle_rsa_compute_root(key, s, s as *const __mpz_struct);
        return 1 as libc::c_int;
    } else {
        __gmpz_set_ui(s, 0 as libc::c_int as libc::c_ulong);
