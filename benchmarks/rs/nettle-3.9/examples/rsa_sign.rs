...
unsafe fn main_0(
    ...
    if argc != 2 as libc::c_int {
        werror(
            b"Usage: rsa-sign PRIVATE-KEY < file\n\0" as *const u8 as *const libc::c_char,
        );
    ...
    if nettle_rsa_sha1_sign(&mut key, &mut hash, s.as_mut_ptr()) == 0 {
        werror(b"RSA key too small\n\0" as *const u8 as *const libc::c_char);
        return 0 as libc::c_int;
    }
    if __gmpz_out_str(stdout, 16 as libc::c_int, s.as_mut_ptr() as mpz_srcptr) == 0 {
        werror(
            b"Failed writing signature: %s\n\0" as *const u8 as *const libc::c_char,
            strerror(*__errno_location()),
        );
        return 0 as libc::c_int;
    }
