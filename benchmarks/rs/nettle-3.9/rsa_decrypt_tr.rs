...
pub unsafe extern "C" fn nettle_rsa_decrypt_tr(
    ...
    let mut key_limb_size: mp_size_t = 0;
    ...
    key_limb_size = __gmpz_size(((*pub_0).n).as_ptr()) as mp_size_t;
    tmp_m_size = key_limb_size as size_t;
    m = _nettle_gmp_alloc(
        (::std::mem::size_of::<mp_limb_t>() as libc::c_ulong)
            .wrapping_mul(key_limb_size as libc::c_ulong),
    ) as *mut mp_limb_t;
    tmp_em_size = (*key).size;
    em = _nettle_gmp_alloc(
        (::std::mem::size_of::<uint8_t>() as libc::c_ulong).wrapping_mul((*key).size),
    ) as *mut uint8_t;
    _nettle_mpz_limbs_copy(m, gibberish, key_limb_size);
    res = _nettle_rsa_sec_compute_root_tr(pub_0, key, random_ctx, random, m, m);
    _nettle_mpn_get_base256(em, (*key).size, m, key_limb_size);
    res &= _nettle_pkcs1_sec_decrypt_variable(length, message, (*key).size, em);
