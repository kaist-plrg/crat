...
unsafe extern "C" fn do_fnmatch(
    ...
    if ((*api).api_get_argument)
        .unwrap()(ext_id, 0 as libc::c_int as size_t, AWK_STRING, &mut pattern) as u64
        == 0
    {
        ((*api).api_warning)
            .unwrap()(
        ...
        .unwrap()(ext_id, 1 as libc::c_int as size_t, AWK_STRING, &mut string) as u64
        == 0
    {
        ((*api).api_warning)
            .unwrap()(
        ...
        .unwrap()(ext_id, 2 as libc::c_int as size_t, AWK_NUMBER, &mut flags) as u64 == 0
    {
        ((*api).api_warning)
            .unwrap()(
            ext_id,
...
unsafe extern "C" fn init_fnmatch() -> awk_bool_t {
    ...
    new_array = ((*api).api_create_array).unwrap()(ext_id);
    i = 0 as libc::c_int;
    while !(flagtable[i as usize].name).is_null() {
        ...
        if ((*api).api_set_array_element)
            .unwrap()(ext_id, new_array, &mut index, &mut value) as u64 == 0
        {
            ((*api).api_warning)
                .unwrap()(
                ext_id,
