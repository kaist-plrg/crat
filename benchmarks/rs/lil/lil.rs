...
unsafe extern "C" fn alloc_value(mut str: *const libc::c_char) -> lil_value_t {
    ...
    if !str.is_null() {
        (*val).l = strlen(str);
        (*val)
            .d = malloc(((*val).l).wrapping_add(1 as libc::c_int as libc::c_ulong))
            as *mut libc::c_char;
        if ((*val).d).is_null() {
...
pub unsafe extern "C" fn lil_set_var(
    ...
    if local != 2 as libc::c_int {
            ...
            && ((*lil).callback[6 as libc::c_int as usize]).is_some()
        {
            ...
            let mut r: libc::c_int = proc_0.unwrap()(lil, name, &mut newval);
    ...
    *fresh8 = if freeval != 0 { val } else { lil_clone_value(val) };
    let fresh9 = (*env).vars;
...
pub unsafe extern "C" fn lil_get_var_or(
        ...
        && (var.is_null() || (*var).env == (*lil).rootenv)
    {
        ...
        if proc_0.unwrap()(lil, name, &mut newretval) != 0 {
            retval = newretval;
        }
    }
    return retval;
...
pub unsafe extern "C" fn lil_parse(
        ...
        && (*lil).parse_depth == 1 as libc::c_int as libc::c_ulong
    {
        ...
        proc_0.unwrap()(lil, (*lil).err_head, (*lil).err_msg);
    }
    if !words.is_null() {
        lil_free_list(words);
    }
...
unsafe extern "C" fn fnc_func(
    ...
    if argc == 3 as libc::c_int as libc::c_ulong {
        ...
        (*cmd)
            .argnames = lil_subst_to_list(lil, *argv.offset(1 as libc::c_int as isize));
...
unsafe extern "C" fn fnc_write(
    ...
    if ((*lil).callback[1 as libc::c_int as usize]).is_some() {
        ...
        proc_0.unwrap()(lil, lil_to_string(msg));
    } else {
...
unsafe extern "C" fn fnc_return(
    ...
    (*(*lil).env)
        .retval = if argc < 1 as libc::c_int as libc::c_ulong {
        0 as lil_value_t
    } else {
...
unsafe extern "C" fn fnc_read(
    ...
    if ((*lil).callback[2 as libc::c_int as usize]).is_some() {
        ...
        buffer = proc_0
            .unwrap()(lil, lil_to_string(*argv.offset(0 as libc::c_int as isize)));
    } else {
        f = fopen(
            lil_to_string(*argv.offset(0 as libc::c_int as isize)),
