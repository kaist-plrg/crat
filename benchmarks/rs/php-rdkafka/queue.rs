...
static mut arginfo_class_RdKafka_Queue___construct: [zend_internal_arg_info; 1] = [
    {
...
static mut arginfo_class_RdKafka_Queue_consume: [zend_internal_arg_info; 2] = unsafe {
    [
        {
            let mut init = _zend_internal_arg_info {
...
static mut class_RdKafka_Queue_methods: [zend_function_entry; 3] = [zend_function_entry {
...
unsafe extern "C" fn register_class_RdKafka_Queue() -> *mut zend_class_entry {
    ...
    ce
        ...
        .unwrap()(
        b"RdKafka\\Queue\0" as *const u8 as *const libc::c_char,
        (::std::mem::size_of::<[libc::c_char; 14]>() as libc::c_ulong)
            .wrapping_sub(1 as libc::c_int as libc::c_ulong),
        1 as libc::c_int,
    );
    ce.info.internal.builtin_functions = class_RdKafka_Queue_methods.as_ptr();
    class_entry = zend_register_internal_class_ex(&mut ce, 0 as *mut zend_class_entry);
    return class_entry;
}
pub static mut ce_kafka_queue: *mut zend_class_entry = 0 as *const zend_class_entry
...
unsafe extern "C" fn kafka_queue_free(mut object: *mut zend_object) {
    let mut intern: *mut kafka_queue_object = (object as *mut libc::c_char)
        .offset(-(24 as libc::c_ulong as isize)) as *mut kafka_queue_object;
    if !((*intern).rkqu).is_null() {
        let mut kafka_intern: *mut kafka_object = get_kafka_object(&mut (*intern).zrk);
        if !kafka_intern.is_null() {
            zend_hash_index_del(&mut (*kafka_intern).queues, intern as zend_ulong);
        }
    }
    zend_object_std_dtor(&mut (*intern).std);
}
unsafe extern "C" fn kafka_queue_new(
    mut class_type: *mut zend_class_entry,
) -> *mut zend_object {
    let mut retval: *mut zend_object = 0 as *mut zend_object;
    let mut intern: *mut kafka_queue_object = 0 as *mut kafka_queue_object;
    intern = zend_object_alloc(
        ::std::mem::size_of::<kafka_queue_object>() as libc::c_ulong,
        class_type,
    ) as *mut kafka_queue_object;
...
pub unsafe extern "C" fn get_kafka_queue_object(
    mut zrkqu: *mut zval,
) -> *mut kafka_queue_object {
    let mut orkqu: *mut kafka_queue_object = ((*zrkqu).value.obj as *mut libc::c_char)
        .offset(-(24 as libc::c_ulong as isize)) as *mut kafka_queue_object;
    if ((*orkqu).rkqu).is_null() {
        zend_throw_exception_ex(
            0 as *mut zend_class_entry,
            0 as libc::c_int as zend_long,
            b"RdKafka\\Queue::__construct() has not been called\0" as *const u8
                as *const libc::c_char,
        );
        return 0 as *mut kafka_queue_object;
    }
    return orkqu;
}
pub unsafe extern "C" fn zim_RdKafka_Queue_consume(
    ...
    message = rd_kafka_consume_queue((*intern).rkqu, timeout_ms as libc::c_int);
    if message.is_null() {
...
pub unsafe extern "C" fn kafka_queue_minit(
    ...
    handlers
        .free_obj = Some(
        kafka_queue_free as unsafe extern "C" fn(*mut zend_object) -> (),
    );
    ...
    class_RdKafka_Queue_methods = [
        {
            let mut init = _zend_function_entry {
                ...
                arg_info: arginfo_class_RdKafka_Queue___construct.as_ptr(),
        ...
        },
        {
            let mut init = _zend_function_entry {
                ...
                handler: Some(
                    zim_RdKafka_Queue_consume
                        as unsafe extern "C" fn(*mut zend_execute_data, *mut zval) -> (),
                ),
                arg_info: arginfo_class_RdKafka_Queue_consume.as_ptr(),
