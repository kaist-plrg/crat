...
pub static mut nist_transition: [transition_t; 10] = [transition_t {
...
pub static mut nist_fsm: osip_statemachine_t = unsafe {
    {
        let mut init = osip_statemachine {
            transitions: nist_transition.as_ptr() as *mut _,
        };
        init
    }
};
unsafe extern "C" fn nist_handle_transport_error(
    mut nist: *mut osip_transaction_t,
    mut err: libc::c_int,
) {
    __osip_transport_error_callback(OSIP_NIST_TRANSPORT_ERROR as libc::c_int, nist, err);
    __osip_transaction_set_state(nist, NIST_TERMINATED);
    __osip_kill_transaction_callback(OSIP_NIST_KILL_TRANSACTION as libc::c_int, nist);
}
pub unsafe extern "C" fn nist_rcv_request(
    mut nist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    let mut i: libc::c_int = 0;
    if (*nist).state as libc::c_uint == NIST_PRE_TRYING as libc::c_int as libc::c_uint {
                ...
                )
        {
            __osip_message_callback(
                OSIP_NIST_REGISTER_RECEIVED as libc::c_int,
                nist,
                (*nist).orig_request,
            );
        ...
        {
            __osip_message_callback(
                OSIP_NIST_BYE_RECEIVED as libc::c_int,
                nist,
                (*nist).orig_request,
            );
        } else if (*(*evt).sip).status_code == 0 as libc::c_int
        ...
        {
            __osip_message_callback(
                OSIP_NIST_OPTIONS_RECEIVED as libc::c_int,
                nist,
                (*nist).orig_request,
            );
        } else if (*(*evt).sip).status_code == 0 as libc::c_int
        ...
        {
            __osip_message_callback(
                OSIP_NIST_INFO_RECEIVED as libc::c_int,
                nist,
                (*nist).orig_request,
            );
        } else if (*(*evt).sip).status_code == 0 as libc::c_int
        ...
        {
            __osip_message_callback(
                OSIP_NIST_CANCEL_RECEIVED as libc::c_int,
                nist,
                (*nist).orig_request,
            );
        } else if (*(*evt).sip).status_code == 0 as libc::c_int
        ...
        {
            __osip_message_callback(
                OSIP_NIST_NOTIFY_RECEIVED as libc::c_int,
                nist,
                (*nist).orig_request,
            );
        } else if (*(*evt).sip).status_code == 0 as libc::c_int
        ...
        {
            __osip_message_callback(
                OSIP_NIST_SUBSCRIBE_RECEIVED as libc::c_int,
                nist,
                (*nist).orig_request,
            );
        } else {
            __osip_message_callback(
                OSIP_NIST_UNKNOWN_REQUEST_RECEIVED as libc::c_int,
                nist,
                (*nist).orig_request,
            );
        }
        ...
        __osip_message_callback(
            OSIP_NIST_REQUEST_RECEIVED_AGAIN as libc::c_int,
            nist,
            (*nist).orig_request,
        );
        if !((*nist).last_response).is_null() {
            i = __osip_transaction_snd_xxx(nist, (*nist).last_response);
            if i != 0 as libc::c_int {
                nist_handle_transport_error(nist, i);
                return;
            } else {
                if (*(*nist).last_response).status_code >= 100 as libc::c_int
                    && (*(*nist).last_response).status_code < 200 as libc::c_int
                {
                    __osip_message_callback(
                        OSIP_NIST_STATUS_1XX_SENT as libc::c_int,
                        nist,
                        (*nist).last_response,
                    );
                } else if (*(*nist).last_response).status_code >= 200 as libc::c_int
                    && (*(*nist).last_response).status_code < 300 as libc::c_int
                {
                    __osip_message_callback(
                        OSIP_NIST_STATUS_2XX_SENT_AGAIN as libc::c_int,
                        nist,
                        (*nist).last_response,
                    );
                } else {
                    __osip_message_callback(
                        OSIP_NIST_STATUS_3456XX_SENT_AGAIN as libc::c_int,
                        nist,
                        (*nist).last_response,
                    );
                }
...
pub unsafe extern "C" fn nist_snd_1xx(
    mut nist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    let mut i: libc::c_int = 0;
...
pub unsafe extern "C" fn nist_snd_23456xx(
    mut nist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    let mut i: libc::c_int = 0;
    ...
    if i != 0 as libc::c_int {
        ...
        == SND_STATUS_2XX as libc::c_int as libc::c_uint
    {
        __osip_message_callback(
            OSIP_NIST_STATUS_2XX_SENT as libc::c_int,
            nist,
            (*nist).last_response,
        );
    } else if (*(*nist).last_response).status_code >= 300 as libc::c_int
        && (*(*nist).last_response).status_code < 400 as libc::c_int
    {
        __osip_message_callback(
            OSIP_NIST_STATUS_3XX_SENT as libc::c_int,
            nist,
            (*nist).last_response,
        );
    } else if (*(*nist).last_response).status_code >= 400 as libc::c_int
        && (*(*nist).last_response).status_code < 500 as libc::c_int
    {
        __osip_message_callback(
            OSIP_NIST_STATUS_4XX_SENT as libc::c_int,
            nist,
            (*nist).last_response,
        );
    } else if (*(*nist).last_response).status_code >= 500 as libc::c_int
        && (*(*nist).last_response).status_code < 600 as libc::c_int
    {
        __osip_message_callback(
            OSIP_NIST_STATUS_5XX_SENT as libc::c_int,
            nist,
            (*nist).last_response,
        );
    } else {
        __osip_message_callback(
            OSIP_NIST_STATUS_6XX_SENT as libc::c_int,
            nist,
            (*nist).last_response,
        );
    }
    if (*nist).state as libc::c_uint != NIST_COMPLETED as libc::c_int as libc::c_uint {
        osip_gettimeofday(
            &mut (*(*nist).nist_context).timer_j_start,
...
pub unsafe extern "C" fn osip_nist_timeout_j_event(
    mut nist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    (*(*nist).nist_context).timer_j_length = -(1 as libc::c_int);
    (*(*nist).nist_context).timer_j_start.tv_sec = -(1 as libc::c_int) as __time_t;
    __osip_transaction_set_state(nist, NIST_TERMINATED);
    __osip_kill_transaction_callback(OSIP_NIST_KILL_TRANSACTION as libc::c_int, nist);
}
