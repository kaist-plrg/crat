...
pub unsafe extern "C" fn __osip_nist_init(
    mut nist: *mut *mut osip_nist_t,
    mut osip: *mut osip_t,
    mut invite: *mut osip_message_t,
) -> libc::c_int {
    let mut i: libc::c_int = 0;
    osip_trace(
        b"nist.c\0" as *const u8 as *const libc::c_char,
        29 as libc::c_int,
        TRACE_LEVEL5,
        0 as *mut FILE,
        b"allocating NIST context\n\0" as *const u8 as *const libc::c_char,
    );
    *nist = (if osip_malloc_func.is_some() {
        osip_malloc_func.unwrap()(::std::mem::size_of::<osip_nist_t>() as libc::c_ulong)
    } else {
        malloc(::std::mem::size_of::<osip_nist_t>() as libc::c_ulong)
    }) as *mut osip_nist_t;
    if (*nist).is_null() {
    ...
    memset(
        *nist as *mut libc::c_void,
        0 as libc::c_int,
        ::std::mem::size_of::<osip_nist_t>() as libc::c_ulong,
    );
    ...
    if i < 0 as libc::c_int {
        if !(*nist).is_null() {
            if osip_free_func.is_some() {
                osip_free_func.unwrap()(*nist as *mut libc::c_void);
            } else {
                free(*nist as *mut libc::c_void);
            }
        }
        *nist = 0 as *mut osip_nist_t;
        return i;
    }
    ...
    if osip_strcasecmp(proto, b"TCP\0" as *const u8 as *const libc::c_char)
        ...
        && osip_strcasecmp(proto, b"SCTP\0" as *const u8 as *const libc::c_char)
            != 0 as libc::c_int
    {
        (**nist).timer_j_length = 64 as libc::c_int * 500 as libc::c_int;
        (**nist).timer_j_start.tv_sec = -(1 as libc::c_int) as __time_t;
    } else {
        (**nist).timer_j_length = 0 as libc::c_int;
        (**nist).timer_j_start.tv_sec = -(1 as libc::c_int) as __time_t;
    }
    return 0 as libc::c_int;
}
pub unsafe extern "C" fn __osip_nist_free(mut nist: *mut osip_nist_t) -> libc::c_int {
    if nist.is_null() {
        return 0 as libc::c_int;
    }
    osip_trace(
...
pub unsafe extern "C" fn __osip_nist_need_timer_j_event(
    mut nist: *mut osip_nist_t,
    mut state: state_t,
    mut transactionid: libc::c_int,
) -> *mut osip_event_t {
    return __osip_transaction_need_timer_x_event(
        nist as *mut libc::c_void,
        &mut (*nist).timer_j_start,
        (state as libc::c_uint == NIST_COMPLETED as libc::c_int as libc::c_uint)
            as libc::c_int,
        transactionid,
        TIMEOUT_J as libc::c_int,
    );
}
