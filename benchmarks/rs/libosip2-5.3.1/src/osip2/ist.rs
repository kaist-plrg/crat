...
pub unsafe extern "C" fn __osip_ist_init(
    mut ist: *mut *mut osip_ist_t,
    mut osip: *mut osip_t,
    mut invite: *mut osip_message_t,
) -> libc::c_int {
    let mut i: libc::c_int = 0;
    osip_trace(
        b"ist.c\0" as *const u8 as *const libc::c_char,
        28 as libc::c_int,
        TRACE_LEVEL5,
        0 as *mut FILE,
        b"allocating IST context\n\0" as *const u8 as *const libc::c_char,
    );
    *ist = (if osip_malloc_func.is_some() {
        osip_malloc_func.unwrap()(::std::mem::size_of::<osip_ist_t>() as libc::c_ulong)
    } else {
        malloc(::std::mem::size_of::<osip_ist_t>() as libc::c_ulong)
    }) as *mut osip_ist_t;
    if (*ist).is_null() {
    ...
    memset(
        *ist as *mut libc::c_void,
        0 as libc::c_int,
        ::std::mem::size_of::<osip_ist_t>() as libc::c_ulong,
    );
    ...
    if i < 0 as libc::c_int {
        if !(*ist).is_null() {
            if osip_free_func.is_some() {
                osip_free_func.unwrap()(*ist as *mut libc::c_void);
            } else {
                free(*ist as *mut libc::c_void);
            }
        }
        *ist = 0 as *mut osip_ist_t;
        return i;
    }
    ...
    if osip_strcasecmp(proto, b"TCP\0" as *const u8 as *const libc::c_char)
        ...
        && osip_strcasecmp(proto, b"SCTP\0" as *const u8 as *const libc::c_char)
            != 0 as libc::c_int
    {
        (**ist).timer_g_length = 500 as libc::c_int;
        (**ist).timer_i_length = 5000 as libc::c_int;
        (**ist).timer_g_start.tv_sec = -(1 as libc::c_int) as __time_t;
        (**ist).timer_i_start.tv_sec = -(1 as libc::c_int) as __time_t;
    } else {
        (**ist).timer_g_length = -(1 as libc::c_int);
        (**ist).timer_i_length = 0 as libc::c_int;
        (**ist).timer_g_start.tv_sec = -(1 as libc::c_int) as __time_t;
        (**ist).timer_i_start.tv_sec = -(1 as libc::c_int) as __time_t;
    }
    (**ist).timer_h_length = 64 as libc::c_int * 500 as libc::c_int;
    (**ist).timer_h_start.tv_sec = -(1 as libc::c_int) as __time_t;
    return 0 as libc::c_int;
}
pub unsafe extern "C" fn __osip_ist_free(mut ist: *mut osip_ist_t) -> libc::c_int {
    if ist.is_null() {
        return 0 as libc::c_int;
    }
    osip_trace(
...
pub unsafe extern "C" fn __osip_ist_need_timer_g_event(
    mut ist: *mut osip_ist_t,
    mut state: state_t,
    mut transactionid: libc::c_int,
) -> *mut osip_event_t {
    return __osip_transaction_need_timer_x_event(
        ist as *mut libc::c_void,
        &mut (*ist).timer_g_start,
        (state as libc::c_uint == IST_COMPLETED as libc::c_int as libc::c_uint)
            as libc::c_int,
        transactionid,
        TIMEOUT_G as libc::c_int,
    );
}
pub unsafe extern "C" fn __osip_ist_need_timer_h_event(
    mut ist: *mut osip_ist_t,
    mut state: state_t,
    mut transactionid: libc::c_int,
) -> *mut osip_event_t {
    return __osip_transaction_need_timer_x_event(
        ist as *mut libc::c_void,
        &mut (*ist).timer_h_start,
        (state as libc::c_uint == IST_COMPLETED as libc::c_int as libc::c_uint)
            as libc::c_int,
        transactionid,
        TIMEOUT_H as libc::c_int,
    );
}
pub unsafe extern "C" fn __osip_ist_need_timer_i_event(
    mut ist: *mut osip_ist_t,
    mut state: state_t,
    mut transactionid: libc::c_int,
) -> *mut osip_event_t {
    return __osip_transaction_need_timer_x_event(
        ist as *mut libc::c_void,
        &mut (*ist).timer_i_start,
        (state as libc::c_uint == IST_CONFIRMED as libc::c_int as libc::c_uint)
            as libc::c_int,
        transactionid,
        TIMEOUT_I as libc::c_int,
    );
}
