...
extern "C" {
    ...
    fn __osip_transaction_snd_xxx(
        ist: *mut osip_transaction_t,
        msg: *mut osip_message_t,
    ) -> libc::c_int;
}
...
pub static mut ist_transition: [transition_t; 11] = [transition_t {
...
pub static mut ist_fsm: osip_statemachine_t = unsafe {
    {
        let mut init = osip_statemachine {
            transitions: ist_transition.as_ptr() as *mut _,
        };
        init
    }
};
unsafe extern "C" fn ist_handle_transport_error(
    mut ist: *mut osip_transaction_t,
    mut err: libc::c_int,
) {
    __osip_transport_error_callback(OSIP_IST_TRANSPORT_ERROR as libc::c_int, ist, err);
    __osip_transaction_set_state(ist, IST_TERMINATED);
    __osip_kill_transaction_callback(OSIP_IST_KILL_TRANSACTION as libc::c_int, ist);
}
pub unsafe extern "C" fn ist_rcv_invite(
    mut ist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    let mut i: libc::c_int = 0;
    if (*ist).state as libc::c_uint == IST_PRE_PROCEEDING as libc::c_int as libc::c_uint
    {
        (*ist).orig_request = (*evt).sip;
        __osip_message_callback(
            OSIP_IST_INVITE_RECEIVED as libc::c_int,
            ist,
            (*evt).sip,
        );
    } else {
        ...
        __osip_message_callback(
            OSIP_IST_INVITE_RECEIVED_AGAIN as libc::c_int,
            ist,
            (*ist).orig_request,
        );
        if !((*ist).last_response).is_null() {
            i = __osip_transaction_snd_xxx(ist, (*ist).last_response);
            if i != 0 as libc::c_int {
                ist_handle_transport_error(ist, i);
                return;
            } else if (*(*ist).last_response).status_code >= 100 as libc::c_int
                && (*(*ist).last_response).status_code < 200 as libc::c_int
            {
                __osip_message_callback(
                    OSIP_IST_STATUS_1XX_SENT as libc::c_int,
                    ist,
                    (*ist).last_response,
                );
            } else if (*(*ist).last_response).status_code >= 200 as libc::c_int
                && (*(*ist).last_response).status_code < 300 as libc::c_int
            {
                __osip_message_callback(
                    OSIP_IST_STATUS_2XX_SENT_AGAIN as libc::c_int,
                    ist,
                    (*ist).last_response,
                );
            } else {
                __osip_message_callback(
                    OSIP_IST_STATUS_3456XX_SENT_AGAIN as libc::c_int,
                    ist,
                    (*ist).last_response,
                );
            }
...
pub unsafe extern "C" fn osip_ist_timeout_g_event(
    mut ist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    let mut i: libc::c_int = 0;
    (*(*ist).ist_context)
        .timer_g_length = (*(*ist).ist_context).timer_g_length * 2 as libc::c_int;
    if (*(*ist).ist_context).timer_g_length > 4000 as libc::c_int {
        (*(*ist).ist_context).timer_g_length = 4000 as libc::c_int;
    }
    osip_gettimeofday(&mut (*(*ist).ist_context).timer_g_start, 0 as *mut libc::c_void);
    add_gettimeofday(
        &mut (*(*ist).ist_context).timer_g_start,
...
pub unsafe extern "C" fn osip_ist_timeout_h_event(
    mut ist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    (*(*ist).ist_context).timer_h_length = -(1 as libc::c_int);
    (*(*ist).ist_context).timer_h_start.tv_sec = -(1 as libc::c_int) as __time_t;
    __osip_transaction_set_state(ist, IST_TERMINATED);
    __osip_kill_transaction_callback(OSIP_IST_KILL_TRANSACTION as libc::c_int, ist);
}
pub unsafe extern "C" fn osip_ist_timeout_i_event(
    mut ist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    (*(*ist).ist_context).timer_i_length = -(1 as libc::c_int);
    (*(*ist).ist_context).timer_i_start.tv_sec = -(1 as libc::c_int) as __time_t;
    __osip_transaction_set_state(ist, IST_TERMINATED);
    __osip_kill_transaction_callback(OSIP_IST_KILL_TRANSACTION as libc::c_int, ist);
}
pub unsafe extern "C" fn ist_snd_1xx(
    mut ist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    let mut i: libc::c_int = 0;
...
pub unsafe extern "C" fn ist_snd_2xx(
    mut ist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    let mut i: libc::c_int = 0;
    ...
    if i != 0 as libc::c_int {
        ...
        __osip_message_callback(
            OSIP_IST_STATUS_2XX_SENT as libc::c_int,
            ist,
            (*ist).last_response,
        );
        __osip_transaction_set_state(ist, IST_TERMINATED);
...
pub unsafe extern "C" fn ist_snd_3456xx(
    mut ist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    let mut i: libc::c_int = 0;
    ...
    if i != 0 as libc::c_int {
        ist_handle_transport_error(ist, i);
        return;
    } else if (*(*ist).last_response).status_code >= 300 as libc::c_int
        && (*(*ist).last_response).status_code < 400 as libc::c_int
    {
        __osip_message_callback(
            OSIP_IST_STATUS_3XX_SENT as libc::c_int,
            ist,
            (*ist).last_response,
        );
    } else if (*(*ist).last_response).status_code >= 400 as libc::c_int
        && (*(*ist).last_response).status_code < 500 as libc::c_int
    {
        __osip_message_callback(
            OSIP_IST_STATUS_4XX_SENT as libc::c_int,
            ist,
            (*ist).last_response,
        );
    } else if (*(*ist).last_response).status_code >= 500 as libc::c_int
        && (*(*ist).last_response).status_code < 600 as libc::c_int
    {
        __osip_message_callback(
            OSIP_IST_STATUS_5XX_SENT as libc::c_int,
            ist,
            (*ist).last_response,
        );
    } else {
        __osip_message_callback(
            OSIP_IST_STATUS_6XX_SENT as libc::c_int,
            ist,
            (*ist).last_response,
        );
    }
    if (*(*ist).ist_context).timer_g_length != -(1 as libc::c_int) {
        osip_gettimeofday(
            &mut (*(*ist).ist_context).timer_g_start,
    ...
    osip_gettimeofday(&mut (*(*ist).ist_context).timer_h_start, 0 as *mut libc::c_void);
    add_gettimeofday(
        &mut (*(*ist).ist_context).timer_h_start,
        (*(*ist).ist_context).timer_h_length,
    );
    __osip_transaction_set_state(ist, IST_COMPLETED);
}
pub unsafe extern "C" fn ist_rcv_ack(
    mut ist: *mut osip_transaction_t,
    mut evt: *mut osip_event_t,
) {
    if !((*ist).ack).is_null() {
    ...
    (*ist).ack = (*evt).sip;
    if (*ist).state as libc::c_uint == IST_COMPLETED as libc::c_int as libc::c_uint {
        __osip_message_callback(OSIP_IST_ACK_RECEIVED as libc::c_int, ist, (*ist).ack);
    } else {
        __osip_message_callback(
            OSIP_IST_ACK_RECEIVED_AGAIN as libc::c_int,
            ist,
            (*ist).ack,
        );
    }
    osip_gettimeofday(&mut (*(*ist).ist_context).timer_i_start, 0 as *mut libc::c_void);
    add_gettimeofday(
        &mut (*(*ist).ist_context).timer_i_start,
    ...
    ist_transition = [
        {
            let mut init = _transition_t {
                ...
                next: &mut *ist_transition.as_mut_ptr().offset(1 as libc::c_int as isize)
                    as *mut transition_t,
                parent: 0 as *mut _transition_t,
            };
            init
        },
        {
            let mut init = _transition_t {
                ...
                next: &mut *ist_transition.as_mut_ptr().offset(2 as libc::c_int as isize)
                    as *mut transition_t,
                parent: 0 as *mut _transition_t,
            };
            init
        },
        {
            let mut init = _transition_t {
                ...
                next: &mut *ist_transition.as_mut_ptr().offset(3 as libc::c_int as isize)
                    as *mut transition_t,
                parent: 0 as *mut _transition_t,
            };
            init
        },
        {
            let mut init = _transition_t {
                ...
                next: &mut *ist_transition.as_mut_ptr().offset(4 as libc::c_int as isize)
                    as *mut transition_t,
                parent: 0 as *mut _transition_t,
            };
            init
        },
        {
            let mut init = _transition_t {
                ...
                next: &mut *ist_transition.as_mut_ptr().offset(5 as libc::c_int as isize)
                    as *mut transition_t,
                parent: 0 as *mut _transition_t,
            };
            init
        },
        {
            let mut init = _transition_t {
                ...
                next: &mut *ist_transition.as_mut_ptr().offset(6 as libc::c_int as isize)
                    as *mut transition_t,
                parent: 0 as *mut _transition_t,
            };
            init
        },
        {
            let mut init = _transition_t {
                ...
                next: &mut *ist_transition.as_mut_ptr().offset(7 as libc::c_int as isize)
                    as *mut transition_t,
                parent: 0 as *mut _transition_t,
            };
            init
        },
        {
            let mut init = _transition_t {
                ...
                next: &mut *ist_transition.as_mut_ptr().offset(8 as libc::c_int as isize)
                    as *mut transition_t,
                parent: 0 as *mut _transition_t,
            };
            init
        },
        {
            let mut init = _transition_t {
                ...
                next: &mut *ist_transition.as_mut_ptr().offset(9 as libc::c_int as isize)
                    as *mut transition_t,
                parent: 0 as *mut _transition_t,
            };
            init
