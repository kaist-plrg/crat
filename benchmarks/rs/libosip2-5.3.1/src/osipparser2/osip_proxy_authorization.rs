...
extern "C" {
    ...
    fn osip_authorization_init(header: *mut *mut osip_authorization_t) -> libc::c_int;
    fn osip_authorization_parse(
        header: *mut osip_authorization_t,
...
pub type osip_proxy_authorization_t = osip_authorization_t;
...
pub unsafe extern "C" fn osip_message_set_proxy_authorization(
    mut sip: *mut osip_message_t,
    mut hvalue: *const libc::c_char,
) -> libc::c_int {
    let mut proxy_authorization: *mut osip_proxy_authorization_t = 0
        as *mut osip_proxy_authorization_t;
    ...
    i = osip_authorization_init(&mut proxy_authorization);
    ...
    osip_list_add(
        ...
        proxy_authorization as *mut libc::c_void,
        -(1 as libc::c_int),
    );
    return 0 as libc::c_int;
}
pub unsafe extern "C" fn osip_message_get_proxy_authorization(
    mut sip: *const osip_message_t,
    mut pos: libc::c_int,
    mut dest: *mut *mut osip_proxy_authorization_t,
) -> libc::c_int {
    let mut proxy_authorization: *mut osip_proxy_authorization_t = 0
        as *mut osip_proxy_authorization_t;
    *dest = 0 as *mut osip_proxy_authorization_t;
    if osip_list_size(&(*sip).proxy_authorizations) <= pos {
        return -(1 as libc::c_int);
    }
    proxy_authorization = osip_list_get(&(*sip).proxy_authorizations, pos)
        as *mut osip_proxy_authorization_t;
