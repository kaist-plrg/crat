...
pub type osip_error_info_t = osip_call_info_t;
...
pub unsafe extern "C" fn osip_message_set_error_info(
    mut sip: *mut osip_message_t,
    mut hvalue: *const libc::c_char,
) -> libc::c_int {
    let mut error_info: *mut osip_error_info_t = 0 as *mut osip_error_info_t;
    ...
    i = osip_call_info_init(&mut error_info);
    ...
    osip_list_add(
        &mut (*sip).error_infos,
        error_info as *mut libc::c_void,
        -(1 as libc::c_int),
    );
    return 0 as libc::c_int;
}
pub unsafe extern "C" fn osip_message_get_error_info(
    mut sip: *const osip_message_t,
    mut pos: libc::c_int,
    mut dest: *mut *mut osip_error_info_t,
) -> libc::c_int {
    let mut error_info: *mut osip_error_info_t = 0 as *mut osip_error_info_t;
    *dest = 0 as *mut osip_error_info_t;
    if osip_list_size(&(*sip).error_infos) <= pos {
        return -(1 as libc::c_int);
    }
    error_info = osip_list_get(&(*sip).error_infos, pos) as *mut osip_error_info_t;
    *dest = error_info;
    return pos;
}
