...
pub unsafe extern "C" fn osip_message_set_call_info(
    mut sip: *mut osip_message_t,
    mut hvalue: *const libc::c_char,
) -> libc::c_int {
    let mut call_info: *mut osip_call_info_t = 0 as *mut osip_call_info_t;
    ...
    i = osip_call_info_init(&mut call_info);
...
pub unsafe extern "C" fn osip_message_get_call_info(
    mut sip: *const osip_message_t,
    mut pos: libc::c_int,
    mut dest: *mut *mut osip_call_info_t,
) -> libc::c_int {
    let mut call_info: *mut osip_call_info_t = 0 as *mut osip_call_info_t;
    *dest = 0 as *mut osip_call_info_t;
    if osip_list_size(&(*sip).call_infos) <= pos {
        return -(1 as libc::c_int);
    }
    call_info = osip_list_get(&(*sip).call_infos, pos) as *mut osip_call_info_t;
    *dest = call_info;
    return pos;
}
pub unsafe extern "C" fn osip_call_info_init(
    mut call_info: *mut *mut osip_call_info_t,
) -> libc::c_int {
    *call_info = (if osip_malloc_func.is_some() {
        osip_malloc_func
            .unwrap()(::std::mem::size_of::<osip_call_info_t>() as libc::c_ulong)
    } else {
        malloc(::std::mem::size_of::<osip_call_info_t>() as libc::c_ulong)
    }) as *mut osip_call_info_t;
    if (*call_info).is_null() {
        return -(4 as libc::c_int);
    }
    (**call_info).element = 0 as *mut libc::c_char;
    osip_list_init(&mut (**call_info).gen_params);
    return 0 as libc::c_int;
}
pub unsafe extern "C" fn osip_call_info_parse(
    mut call_info: *mut osip_call_info_t,
    mut hvalue: *const libc::c_char,
) -> libc::c_int {
    let mut osip_call_info_params: *const libc::c_char = 0 as *const libc::c_char;
    let mut i: libc::c_int = 0;
    osip_call_info_params = strchr(hvalue, '<' as i32);
    if osip_call_info_params.is_null() {
        return -(5 as libc::c_int);
    }
    osip_call_info_params = strchr(
        osip_call_info_params.offset(1 as libc::c_int as isize),
        '>' as i32,
    );
    if osip_call_info_params.is_null() {
        return -(5 as libc::c_int);
    ...
    if !osip_call_info_params.is_null() {
        ...
        osip_call_info_params = hvalue.offset(strlen(hvalue) as isize);
    }
    if (osip_call_info_params.offset_from(hvalue) as libc::c_long
    ...
    (*call_info)
        ...
        osip_malloc_func
            .unwrap()(
            (osip_call_info_params.offset_from(hvalue) as libc::c_long
    ...
    if ((*call_info).element).is_null() {
        return -(4 as libc::c_int);
    }
    osip_clrncpy(
        (*call_info).element,
        hvalue,
        osip_call_info_params.offset_from(hvalue) as libc::c_long as size_t,
    );
    return 0 as libc::c_int;
}
pub unsafe extern "C" fn osip_call_info_to_str(
    mut call_info: *const osip_call_info_t,
    ...
    *dest = 0 as *mut libc::c_char;
    if call_info.is_null() || ((*call_info).element).is_null() {
        return -(2 as libc::c_int);
    }
    len = (strlen((*call_info).element)).wrapping_add(2 as libc::c_int as libc::c_ulong);
    ...
    *dest = buf;
    sprintf(buf, b"%s\0" as *const u8 as *const libc::c_char, (*call_info).element);
...
pub unsafe extern "C" fn osip_call_info_free(mut call_info: *mut osip_call_info_t) {
    if call_info.is_null() {
        return;
    }
    if !((*call_info).element).is_null() {
        if osip_free_func.is_some() {
            osip_free_func.unwrap()((*call_info).element as *mut libc::c_void);
        } else {
            free((*call_info).element as *mut libc::c_void);
        }
    }
    osip_uri_param_freelist(&mut (*call_info).gen_params);
    (*call_info).element = 0 as *mut libc::c_char;
    if !call_info.is_null() {
        if osip_free_func.is_some() {
            osip_free_func.unwrap()(call_info as *mut libc::c_void);
        } else {
            free(call_info as *mut libc::c_void);
        }
    }
}
pub unsafe extern "C" fn osip_call_info_clone(
    mut ctt: *const osip_call_info_t,
    mut dest: *mut *mut osip_call_info_t,
) -> libc::c_int {
    let mut i: libc::c_int = 0;
    let mut ct: *mut osip_call_info_t = 0 as *mut osip_call_info_t;
    *dest = 0 as *mut osip_call_info_t;
    if ctt.is_null() {
        return -(2 as libc::c_int);
    }
...
pub unsafe extern "C" fn osip_call_info_get_uri(
    mut ae: *mut osip_call_info_t,
) -> *mut libc::c_char {
    return (*ae).element;
}
pub unsafe extern "C" fn osip_call_info_set_uri(
    mut ae: *mut osip_call_info_t,
