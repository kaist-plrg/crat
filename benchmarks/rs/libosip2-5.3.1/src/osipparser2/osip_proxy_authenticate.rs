...
pub type osip_proxy_authenticate_t = osip_www_authenticate_t;
...
pub unsafe extern "C" fn osip_message_set_proxy_authenticate(
    mut sip: *mut osip_message_t,
    mut hvalue: *const libc::c_char,
) -> libc::c_int {
    let mut proxy_authenticate: *mut osip_proxy_authenticate_t = 0
        as *mut osip_proxy_authenticate_t;
    ...
    i = osip_www_authenticate_init(&mut proxy_authenticate);
    ...
    osip_list_add(
        ...
        proxy_authenticate as *mut libc::c_void,
        -(1 as libc::c_int),
    );
    return 0 as libc::c_int;
}
pub unsafe extern "C" fn osip_message_get_proxy_authenticate(
    mut sip: *const osip_message_t,
    mut pos: libc::c_int,
    mut dest: *mut *mut osip_proxy_authenticate_t,
) -> libc::c_int {
    let mut proxy_authenticate: *mut osip_proxy_authenticate_t = 0
        as *mut osip_proxy_authenticate_t;
    *dest = 0 as *mut osip_proxy_authenticate_t;
    if osip_list_size(&(*sip).proxy_authenticates) <= pos {
        return -(1 as libc::c_int);
    }
    proxy_authenticate = osip_list_get(&(*sip).proxy_authenticates, pos)
        as *mut osip_proxy_authenticate_t;
