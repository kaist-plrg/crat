...
extern "C" {
    ...
    fn osip_authentication_info_init(
        header: *mut *mut osip_authentication_info_t,
    ) -> libc::c_int;
    fn osip_authentication_info_parse(
        header: *mut osip_authentication_info_t,
...
pub type osip_proxy_authentication_info_t = osip_authentication_info_t;
...
pub unsafe extern "C" fn osip_message_set_proxy_authentication_info(
    mut sip: *mut osip_message_t,
    mut hvalue: *const libc::c_char,
) -> libc::c_int {
    let mut proxy_authentication_info: *mut osip_proxy_authentication_info_t = 0
        as *mut osip_proxy_authentication_info_t;
    ...
    i = osip_authentication_info_init(&mut proxy_authentication_info);
    if i != 0 as libc::c_int {
        return i;
    }
    i = osip_authentication_info_parse(proxy_authentication_info, hvalue);
    if i != 0 as libc::c_int {
        osip_authentication_info_free(proxy_authentication_info);
        return i;
    }
    (*sip).message_property = 2 as libc::c_int;
    osip_list_add(
        &mut (*sip).proxy_authentication_infos,
        proxy_authentication_info as *mut libc::c_void,
        -(1 as libc::c_int),
    );
    return 0 as libc::c_int;
}
pub unsafe extern "C" fn osip_message_get_proxy_authentication_info(
    mut sip: *const osip_message_t,
    mut pos: libc::c_int,
    mut dest: *mut *mut osip_proxy_authentication_info_t,
) -> libc::c_int {
    let mut proxy_authentication_info: *mut osip_proxy_authentication_info_t = 0
        as *mut osip_proxy_authentication_info_t;
    *dest = 0 as *mut osip_proxy_authentication_info_t;
    if osip_list_size(&(*sip).proxy_authentication_infos) <= pos {
        return -(1 as libc::c_int);
    }
    proxy_authentication_info = osip_list_get(&(*sip).proxy_authentication_infos, pos)
        as *mut osip_proxy_authentication_info_t;
