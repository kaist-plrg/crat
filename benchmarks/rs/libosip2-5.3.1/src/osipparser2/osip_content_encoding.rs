...
pub type osip_content_encoding_t = osip_content_length_t;
#[derive(Copy, Clone)]
#[repr(C)]
...
pub unsafe extern "C" fn osip_message_set_content_encoding(
    mut sip: *mut osip_message_t,
    mut hvalue: *const libc::c_char,
) -> libc::c_int {
    let mut content_encoding: *mut osip_content_encoding_t = 0
        as *mut osip_content_encoding_t;
    ...
    i = osip_content_length_init(&mut content_encoding);
    ...
    osip_list_add(
        ...
        content_encoding as *mut libc::c_void,
        -(1 as libc::c_int),
    );
    return 0 as libc::c_int;
}
pub unsafe extern "C" fn osip_message_get_content_encoding(
    mut sip: *const osip_message_t,
    mut pos: libc::c_int,
    mut dest: *mut *mut osip_content_encoding_t,
) -> libc::c_int {
    let mut ce: *mut osip_content_encoding_t = 0 as *mut osip_content_encoding_t;
    *dest = 0 as *mut osip_content_encoding_t;
    if osip_list_size(&(*sip).content_encodings) <= pos {
        return -(1 as libc::c_int);
    }
    ce = osip_list_get(&(*sip).content_encodings, pos) as *mut osip_content_encoding_t;
    *dest = ce;
    return pos;
}
