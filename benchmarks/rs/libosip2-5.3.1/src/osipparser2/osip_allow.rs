...
pub type osip_allow_t = osip_content_length_t;
#[derive(Copy, Clone)]
#[repr(C)]
...
pub unsafe extern "C" fn osip_message_set_allow(
    mut sip: *mut osip_message_t,
    mut hvalue: *const libc::c_char,
) -> libc::c_int {
    let mut allow: *mut osip_allow_t = 0 as *mut osip_allow_t;
    ...
    i = osip_content_length_init(&mut allow);
    ...
    osip_list_add(&mut (*sip).allows, allow as *mut libc::c_void, -(1 as libc::c_int));
    return 0 as libc::c_int;
}
pub unsafe extern "C" fn osip_message_get_allow(
    mut sip: *const osip_message_t,
    mut pos: libc::c_int,
    mut dest: *mut *mut osip_allow_t,
) -> libc::c_int {
    let mut allow: *mut osip_allow_t = 0 as *mut osip_allow_t;
    *dest = 0 as *mut osip_allow_t;
    if osip_list_size(&(*sip).allows) <= pos {
        return -(1 as libc::c_int);
    }
    allow = osip_list_get(&(*sip).allows, pos) as *mut osip_allow_t;
    *dest = allow;
    return pos;
}
