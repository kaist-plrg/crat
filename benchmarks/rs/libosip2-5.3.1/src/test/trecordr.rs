...
extern "C" {
    ...
    fn osip_record_route_init(header: *mut *mut osip_record_route_t) -> libc::c_int;
    fn osip_record_route_free(header: *mut osip_record_route_t);
    fn osip_record_route_parse(
        header: *mut osip_record_route_t,
...
unsafe fn main_0(
    ...
    a_record_route = (if osip_malloc_func.is_some() {
        osip_malloc_func.unwrap()(200 as libc::c_int as size_t)
    } else {
        malloc(200 as libc::c_int as libc::c_ulong)
    }) as *mut libc::c_char;
    res = fgets(a_record_route, 200 as libc::c_int, record_routes_file);
    while !res.is_null() {
        let mut errcode: libc::c_int = 0;
        strncpy(
            a_record_route
                .offset(strlen(a_record_route) as isize)
        ...
        {
            osip_record_route_init(&mut record_route);
            printf(
                b"=================================================\n\0" as *const u8
                    as *const libc::c_char,
            );
            printf(
                b"RECORD_ROUTE TO PARSE: |%s|\n\0" as *const u8 as *const libc::c_char,
                a_record_route,
            );
            errcode = osip_record_route_parse(record_route, a_record_route);
            if errcode != -(1 as libc::c_int) {
                if osip_record_route_to_str(record_route, &mut dest)
                    != -(1 as libc::c_int)
                {
                    printf(
                        b"result:                |%s|\n\0" as *const u8
    ...
    if !a_record_route.is_null() {
        if osip_free_func.is_some() {
            osip_free_func.unwrap()(a_record_route as *mut libc::c_void);
        } else {
            free(a_record_route as *mut libc::c_void);
        }
    }
