...
unsafe fn main_0(
    ...
    let mut froms_file: *mut FILE = 0 as *mut FILE;
    let mut from: *mut osip_from_t = 0 as *mut osip_from_t;
    let mut a_from: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    a_from = (if osip_malloc_func.is_some() {
        osip_malloc_func.unwrap()(200 as libc::c_int as size_t)
    } else {
        malloc(200 as libc::c_int as libc::c_ulong)
    }) as *mut libc::c_char;
    res = fgets(a_from, 200 as libc::c_int, froms_file);
    while !res.is_null() {
        let mut errcode: libc::c_int = 0;
        strncpy(
            a_from.offset(strlen(a_from) as isize).offset(-(1 as libc::c_int as isize)),
            ...
            )
        {
            ...
            if errcode != -(1 as libc::c_int) {
                if osip_from_to_str(from, &mut dest) != -(1 as libc::c_int) {
                    printf(
                        b"result:        |%s|\n\0" as *const u8 as *const libc::c_char,
                        dest,
                    );
    ...
    if !a_from.is_null() {
        if osip_free_func.is_some() {
            osip_free_func.unwrap()(a_from as *mut libc::c_void);
        } else {
            free(a_from as *mut libc::c_void);
        }
    }
