...
extern "C" {
    ...
    fn osip_route_init(header: *mut *mut osip_route_t) -> libc::c_int;
    fn osip_route_free(header: *mut osip_route_t);
    fn osip_route_parse(
        header: *mut osip_route_t,
...
unsafe fn main_0(
    ...
    let mut routes_file: *mut FILE = 0 as *mut FILE;
    let mut route: *mut osip_route_t = 0 as *mut osip_route_t;
    let mut a_route: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    while !res.is_null() {
            ...
            )
        {
            ...
            if errcode != -(1 as libc::c_int) {
                if osip_route_to_str(route, &mut dest) != -(1 as libc::c_int) {
                    printf(
                        b"result:         |%s|\n\0" as *const u8 as *const libc::c_char,
                        dest,
                    );
    ...
    if !a_route.is_null() {
        if osip_free_func.is_some() {
            osip_free_func.unwrap()(a_route as *mut libc::c_void);
        } else {
            free(a_route as *mut libc::c_void);
        }
    }
