...
unsafe extern "C" fn rsp_make_error(
    ...
    if id != 0 as libc::c_int as libc::c_ulong {
        ...
        while !cmsg.is_null() && (*cmsg).frag_id == id {
            ...
            ((*conn).dequeue_outq).unwrap()(ctx, conn, cmsg);
    ...
    if (*conn).eof() != 0 {
        ...
        if log_loggable(1 as libc::c_int) != 0 as libc::c_int {
            _log(
                ...
                (*conn).sd,
                ((*conn).active).unwrap()(conn) as libc::c_int,
            );
        }
        return 0 as *mut msg;
    }
...
unsafe extern "C" fn rsp_filter(
    ...
    if (*pmsg).swallow() != 0 {
        ((*conn).swallow_msg).unwrap()(conn, pmsg, msg);
...
unsafe extern "C" fn rsp_forward(
    ...
    ((*s_conn).dequeue_outq).unwrap()(ctx, s_conn, pmsg);
    (*pmsg).set_done(1 as libc::c_int as libc::c_uint);
    (*pmsg).peer = msg;
    (*msg).peer = pmsg;
    ((*msg).pre_coalesce).unwrap()(msg);
    c_conn = (*pmsg).owner;
