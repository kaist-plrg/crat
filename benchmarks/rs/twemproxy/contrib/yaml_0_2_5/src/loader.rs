...
pub unsafe extern "C" fn yaml_parser_load(
    ...
    if !(if !((*document).nodes.start).is_null() {
        ...
        (*document)
            .nodes
            .end = ((*document).nodes.start).offset(16 as libc::c_int as isize);
        1 as libc::c_int
    } else {
    ...
    } == 0)
    {
        if (*parser).stream_start_produced == 0 {
            if yaml_parser_parse(parser, &mut event) == 0 {
                current_block = 1230127257082107353;
            } else {
...
unsafe extern "C" fn yaml_parser_load_document(
    ...
    (*(*parser).document)
        .version_directive = (*event).data.document_start.version_directive;
    (*(*parser).document)
        .tag_directives
        .start = (*event).data.document_start.tag_directives.start;
    (*(*parser).document)
        .tag_directives
        ...
        2 => {
                ...
                ) != 0
            {
                ...
                (*parent)
                    ...
                    .sequence
                    .items
                    .top = ((*parent).data.sequence.items.top).offset(1);
                *fresh1 = index;
                1 as libc::c_int
            } else {
                (*parser).error = YAML_MEMORY_ERROR;
                ...
                _ => {
                        ...
                        ) != 0
                    {
                        ...
                        (*parent)
                            ...
                            .mapping
                            .pairs
                            .top = ((*parent).data.mapping.pairs.top).offset(1);
                        *fresh2 = pair;
                        1 as libc::c_int
                    } else {
                        (*parser).error = YAML_MEMORY_ERROR;
...
unsafe extern "C" fn yaml_parser_load_scalar(
    ...
    } == 0)
    {
            ...
            ) == 0 as libc::c_int
        {
            ...
            if tag.is_null() {
                current_block = 7352874724277973185;
            } else {
        ...
        match current_block {
            ...
            _ => {
                    ...
                    ) != 0
                {
                    ...
                    (*(*parser).document)
                        .nodes
                        .top = ((*(*parser).document).nodes.top).offset(1);
                    *fresh3 = node;
                    1 as libc::c_int
                } else {
                    (*parser).error = YAML_MEMORY_ERROR;
...
unsafe extern "C" fn yaml_parser_load_sequence(
    ...
    } == 0)
    {
            ...
            ) == 0 as libc::c_int
        {
            ...
            if tag.is_null() {
                current_block = 1573319214984807142;
            } else {
                current_block = 11875828834189669668;
            }
        } else {
...
unsafe extern "C" fn yaml_parser_load_mapping(
    ...
    } == 0)
    {
            ...
            ) == 0 as libc::c_int
        {
            ...
            if tag.is_null() {
                current_block = 15813062893439906442;
            } else {
                current_block = 11875828834189669668;
            }
        } else {
