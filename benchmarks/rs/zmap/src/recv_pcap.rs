...
extern "C" {
    ...
    fn handle_packet(buflen: uint32_t, bytes: *const uint8_t, ts: timespec);
...
>;
static mut pc: *mut pcap_t = 0 as *const pcap_t as *mut pcap_t;
pub unsafe extern "C" fn packet_cb(
    ...
    if zrecv.filter_success >= zconf.max_results as libc::c_ulong {
        return;
    }
    let mut buflen: uint32_t = (*p).caplen;
    ts.tv_sec = (*p).ts.tv_sec;
    ts.tv_nsec = (*p).ts.tv_usec * 1000 as libc::c_int as libc::c_long;
    handle_packet(buflen, bytes, ts);
}
pub unsafe extern "C" fn recv_init() {
    let mut bpftmp: [libc::c_char; 1024] = [0; 1024];
    let mut errbuf: [libc::c_char; 256] = [0; 256];
    pc = pcap_open_live(
        zconf.iface,
        (*zconf.probe_module).pcap_snaplen as libc::c_int,
        1 as libc::c_int,
        1000 as libc::c_int,
        errbuf.as_mut_ptr(),
    );
    if pc.is_null() {
        log_fatal(
            ...
            b"could not open device %s: %s\0" as *const u8 as *const libc::c_char,
            zconf.iface,
            errbuf.as_mut_ptr(),
        );
    }
    match pcap_datalink(pc) {
        1 => {
            log_debug(
                b"recv\0" as *const u8 as *const libc::c_char,
                b"Data link layer Ethernet\0" as *const u8 as *const libc::c_char,
            );
            zconf
                .data_link_size = ::std::mem::size_of::<ether_header>() as libc::c_ulong
                as libc::c_int;
        }
        12 => {
            log_info(
                b"recv\0" as *const u8 as *const libc::c_char,
                b"Data link RAW\0" as *const u8 as *const libc::c_char,
            );
            zconf.data_link_size = 0 as libc::c_int;
        }
        113 => {
            log_info(
                b"recv\0" as *const u8 as *const libc::c_char,
                b"Data link cooked socket\0" as *const u8 as *const libc::c_char,
            );
            zconf.data_link_size = 16 as libc::c_int;
        }
        _ => {
            log_error(
                b"recv\0" as *const u8 as *const libc::c_char,
                b"unknown data link layer\0" as *const u8 as *const libc::c_char,
            );
        }
    }
    let mut bpf: bpf_program = bpf_program {
        bf_len: 0,
        bf_insns: 0 as *mut bpf_insn,
    };
    if zconf.send_ip_pkts == 0 {
        snprintf(
            ...
            b"not ether src %02x:%02x:%02x:%02x:%02x:%02x\0" as *const u8
        ...
        if (strlen((*zconf.probe_module).pcap_filter))
            .wrapping_add(10 as libc::c_int as libc::c_ulong)
            < (1024 as libc::c_int as libc::c_ulong)
                .wrapping_sub(strlen(bpftmp.as_mut_ptr()))
        {} else {
            __assert_fail(
                b"strlen(zconf.probe_module->pcap_filter) + 10 < (BPFLEN - strlen(bpftmp))\0"
                    as *const u8 as *const libc::c_char,
                b"recv-pcap.c\0" as *const u8 as *const libc::c_char,
        ...
        bpftmp[0 as libc::c_int as usize] = 0 as libc::c_int as libc::c_char;
    }
    if !((*zconf.probe_module).pcap_filter).is_null() {
        if zconf.send_ip_pkts == 0 {
            strcat(bpftmp.as_mut_ptr(), b" and (\0" as *const u8 as *const libc::c_char);
        } else {
            strcat(bpftmp.as_mut_ptr(), b"(\0" as *const u8 as *const libc::c_char);
        }
        strcat(bpftmp.as_mut_ptr(), (*zconf.probe_module).pcap_filter);
        strcat(bpftmp.as_mut_ptr(), b")\0" as *const u8 as *const libc::c_char);
    }
    if strcmp(bpftmp.as_mut_ptr(), b"\0" as *const u8 as *const libc::c_char) != 0 {
        if pcap_compile(
            pc,
            &mut bpf,
            bpftmp.as_mut_ptr(),
            1 as libc::c_int,
            0 as libc::c_int as bpf_u_int32,
        ) < 0 as libc::c_int
        {
            log_fatal(
                b"recv\0" as *const u8 as *const libc::c_char,
                b"couldn't compile filter\0" as *const u8 as *const libc::c_char,
            );
        }
        if pcap_setfilter(pc, &mut bpf) < 0 as libc::c_int {
            log_fatal(
                b"recv\0" as *const u8 as *const libc::c_char,
                b"couldn't install filter\0" as *const u8 as *const libc::c_char,
            );
        }
    }
    if pcap_setnonblock(pc, 1 as libc::c_int, errbuf.as_mut_ptr()) == -(1 as libc::c_int)
    {
        log_fatal(
            b"recv\0" as *const u8 as *const libc::c_char,
            b"pcap_setnonblock error:%s\0" as *const u8 as *const libc::c_char,
            errbuf.as_mut_ptr(),
        );
    }
}
pub unsafe extern "C" fn recv_packets() {
    let mut ret: libc::c_int = pcap_dispatch(
        pc,
        -(1 as libc::c_int),
        Some(
            packet_cb
