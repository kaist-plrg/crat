...
extern "C" {
    ...
    fn udp_global_initialize(conf: *mut state_conf) -> libc::c_int;
...
pub unsafe extern "C" fn ntp_make_packet(
    ...
    let mut ntp: *mut ntphdr = &mut *udp_header.offset(1 as libc::c_int as isize)
        as *mut udphdr as *mut ntphdr;
...
pub unsafe extern "C" fn ntp_process_packet(
    ...
    if (*ip_hdr).ip_p as libc::c_int == IPPROTO_UDP as libc::c_int {
        let mut udp: *mut udphdr = (ip_hdr as *mut libc::c_char)
            ...
            as *mut udphdr;
        let mut ptr: *mut uint8_t = &mut *udp.offset(1 as libc::c_int as isize)
            as *mut udphdr as *mut uint8_t;
        fs_add_string(
            fs,
        ...
        if len > 90 as libc::c_int as libc::c_uint {
            temp8 = *ptr;
            fs_add_uint64(
                fs,
...
pub unsafe extern "C" fn ntp_init_perthread(
    ...
    let mut udp_header: *mut udphdr = &mut *ip_header.offset(1 as libc::c_int as isize)
        as *mut ip as *mut udphdr;
    let mut ntp_header: *mut ntphdr = &mut *udp_header.offset(1 as libc::c_int as isize)
        as *mut udphdr as *mut ntphdr;
...
pub unsafe extern "C" fn ntp_print_packet(
    ...
    let mut iph: *mut ip = &mut *ethh.offset(1 as libc::c_int as isize)
        as *mut ether_header as *mut ip;
    let mut udph: *mut udphdr = iph
        .offset((4 as libc::c_int * (*iph).ip_hl() as libc::c_int) as isize)
        as *mut udphdr;
    let mut ntph: *mut ntphdr = &mut *udph.offset(1 as libc::c_int as isize)
        as *mut udphdr as *mut ntphdr;
    fprintf(
        fp,
...
static mut fields: [fielddef_t; 19] = [
    {
