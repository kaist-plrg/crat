...
pub unsafe extern "C" fn BrotliDecoderStateCleanupAfterMetablock(
    ...
    ((*s).free_func)
        .unwrap()((*s).memory_manager_opaque, (*s).context_modes as *mut libc::c_void);
    (*s).context_modes = 0 as *mut uint8_t;
    ((*s).free_func)
        .unwrap()((*s).memory_manager_opaque, (*s).context_map as *mut libc::c_void);
    (*s).context_map = 0 as *mut uint8_t;
    ((*s).free_func)
        .unwrap()(
...
pub unsafe extern "C" fn BrotliDecoderStateCleanup(
    ...
    ((*s).free_func)
        .unwrap()((*s).memory_manager_opaque, (*s).ringbuffer as *mut libc::c_void);
    (*s).ringbuffer = 0 as *mut uint8_t;
    ((*s).free_func)
        .unwrap()(
        (*s).memory_manager_opaque,
...
pub unsafe extern "C" fn BrotliDecoderHuffmanTreeGroupInit(
    ...
    let code_size: size_t = (::std::mem::size_of::<HuffmanCode>() as libc::c_ulong)
        .wrapping_mul(ntrees as libc::c_ulong)
        .wrapping_mul(max_table_size);
    let htree_size: size_t = (::std::mem::size_of::<*mut HuffmanCode>() as libc::c_ulong)
        .wrapping_mul(ntrees as libc::c_ulong);
    let mut p: *mut *mut HuffmanCode = ((*s).alloc_func)
        .unwrap()((*s).memory_manager_opaque, code_size.wrapping_add(htree_size))
