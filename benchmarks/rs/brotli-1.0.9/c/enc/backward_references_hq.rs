...
unsafe extern "C" fn BackwardMatchLengthCode(
    ...
    let mut code: size_t = ((*self_0).length_and_code
        & 31 as libc::c_int as libc::c_uint) as size_t;
...
unsafe extern "C" fn InitZopfliCostModel(
    ...
    (*self_0)
        .literal_costs_ = if num_bytes.wrapping_add(2 as libc::c_int as libc::c_ulong)
    ...
    (*self_0)
        .cost_dist_ = if (*dist).alphabet_size_limit > 0 as libc::c_int as libc::c_uint {
        BrotliAllocate(
            m,
...
unsafe extern "C" fn UpdateNodes(
    ...
    while k < max_iters && k < StartPosQueueSize(queue) {
        ...
        let base_cost: libc::c_float = start_costdiff
            ...
            + ZopfliCostModelGetLiteralCosts(model, 0 as libc::c_int as size_t, pos);
        let mut best_len: size_t = min_len
...
pub unsafe extern "C" fn BrotliCreateHqZopfliBackwardReferences(
    ...
    let max_backward_limit: size_t = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let mut num_matches: *mut uint32_t = if num_bytes > 0 as libc::c_int as libc::c_ulong
    {
        BrotliAllocate(
            m,
    ...
    let mut matches_size: size_t = (4 as libc::c_int as libc::c_ulong)
        .wrapping_mul(num_bytes);
    let store_end: size_t = if num_bytes >= StoreLookaheadH10() {
        position
            .wrapping_add(num_bytes)
            .wrapping_sub(StoreLookaheadH10())
    ...
    let mut matches: *mut BackwardMatch = if matches_size
        > 0 as libc::c_int as libc::c_ulong
    {
        BrotliAllocate(
            m,
        ...
        .wrapping_sub(1 as libc::c_int as libc::c_ulong) < num_bytes
    {
        ...
        {
            let mut _new_size: size_t = if matches_size
                == 0 as libc::c_int as libc::c_ulong
            {
                cur_match_pos
                    .wrapping_add(128 as libc::c_int as libc::c_ulong)
