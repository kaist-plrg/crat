...
unsafe extern "C" fn StoreH40(
    ...
    let bank: size_t = key & (1 as libc::c_int - 1 as libc::c_int) as libc::c_ulong;
    let fresh0 = (*self_0).free_slot_idx[bank as usize];
    (*self_0)
        .free_slot_idx[bank
        as usize] = ((*self_0).free_slot_idx[bank as usize]).wrapping_add(1);
    let idx: size_t = (fresh0 as libc::c_int
        & ((1 as libc::c_int) << 16 as libc::c_int) - 1 as libc::c_int) as size_t;
    let mut delta: size_t = ix.wrapping_sub(*addr.offset(key as isize) as libc::c_ulong);
...
unsafe extern "C" fn InitializeHROLLING_FAST(
    ...
    while i < 32 as libc::c_int as libc::c_ulong {
        (*self_0)
            .factor_remove = ((*self_0).factor_remove as libc::c_uint)
            .wrapping_mul((*self_0).factor) as uint32_t as uint32_t;
...
unsafe extern "C" fn HasherSetup(
    ...
    let mut one_shot: libc::c_int = (position == 0 as libc::c_int as libc::c_ulong
    ...
    if ((*hasher).common.extra).is_null() {
        ...
        (*hasher)
            .common
            .extra = (if alloc_size > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                alloc_size
...
unsafe extern "C" fn StoreH5(
    ...
    let minor_ix: size_t = (*((*self_0).num_).offset(key as isize) as libc::c_uint
        & (*self_0).block_mask_) as size_t;
    let offset: size_t = minor_ix
        .wrapping_add((key << (*self_0).block_bits_) as libc::c_ulong);
...
unsafe extern "C" fn StoreH41(
    ...
    let bank: size_t = key & (1 as libc::c_int - 1 as libc::c_int) as libc::c_ulong;
    let fresh2 = (*self_0).free_slot_idx[bank as usize];
    (*self_0)
        .free_slot_idx[bank
        as usize] = ((*self_0).free_slot_idx[bank as usize]).wrapping_add(1);
    let idx: size_t = (fresh2 as libc::c_int
        & ((1 as libc::c_int) << 16 as libc::c_int) - 1 as libc::c_int) as size_t;
    let mut delta: size_t = ix.wrapping_sub(*addr.offset(key as isize) as libc::c_ulong);
    *tiny_hash.offset(ix as uint16_t as isize) = key as uint8_t;
    if delta > 0xffff as libc::c_int as libc::c_ulong {
...
unsafe extern "C" fn StoreH42(
    ...
    let bank: size_t = key & (512 as libc::c_int - 1 as libc::c_int) as libc::c_ulong;
...
unsafe extern "C" fn StoreAndFindMatchesH10(
    ...
    let should_reroot_tree: libc::c_int = if max_length
            ...
            || depth_remaining == 0 as libc::c_int as libc::c_ulong
        {
            ...
            if !matches.is_null() && len > *best_len {
                *best_len = len;
                ...
                InitBackwardMatch(fresh5, backward, len);
            }
            if len >= max_comp_len {
                if should_reroot_tree != 0 {
                    *forest
...
unsafe extern "C" fn ShannonEntropy(
    ...
    if size & 1 as libc::c_int as libc::c_ulong != 0 {
        current_block = 4774861200198514387;
    } else {
...
unsafe extern "C" fn RingBufferSetup(
    ...
    *(&mut(*rb).size_ as *const uint32_t
        as *mut uint32_t) = (1 as libc::c_uint) << window_bits;
    *(&mut(*rb).mask_ as *const uint32_t
        as *mut uint32_t) = ((1 as libc::c_uint) << window_bits)
        .wrapping_sub(1 as libc::c_int as libc::c_uint);
    *(&mut(*rb).tail_size_ as *const uint32_t
        as *mut uint32_t) = (1 as libc::c_uint) << tail_bits;
    *(&mut(*rb).total_size_ as *const uint32_t
...
unsafe extern "C" fn RingBufferInitBuffer(
    ...
    let ref mut fresh8 = *((*rb).buffer_).offset(-(1 as libc::c_int) as isize);
    *fresh8 = 0 as libc::c_int as uint8_t;
    *((*rb).buffer_).offset(-(2 as libc::c_int) as isize) = *fresh8;
    i = 0 as libc::c_int as size_t;
...
unsafe extern "C" fn GetBrotliStorage(
    ...
    if (*s).storage_size_ < size {
        ...
        (*s)
            .storage_ = if size > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
...
pub unsafe extern "C" fn BrotliEncoderCreateInstance(
    ...
    if alloc_func.is_none() && free_func.is_none() {
        ...
        state = alloc_func
            .unwrap()(
            opaque,
            ::std::mem::size_of::<BrotliEncoderState>() as libc::c_ulong,
        ) as *mut BrotliEncoderState;
    }
    if state.is_null() {
...
unsafe extern "C" fn EncodeData(
    ...
    let max_literals: size_t = max_length
        .wrapping_div(8 as libc::c_int as libc::c_ulong);
    let max_commands: size_t = max_length
        .wrapping_div(8 as libc::c_int as libc::c_ulong);
    ...
    if (*s).last_insert_len_ > 0 as libc::c_int as libc::c_ulong {
        ...
        (*s)
            .num_literals_ = ((*s).num_literals_ as libc::c_ulong)
            .wrapping_add((*s).last_insert_len_) as size_t as size_t;
        (*s).last_insert_len_ = 0 as libc::c_int as size_t;
    }
...
unsafe extern "C" fn WriteMetadataHeader(
    ...
    *header
    ...
    if block_size == 0 as libc::c_int as libc::c_ulong {
        ...
        let mut nbits: uint32_t = if block_size == 1 as libc::c_int as libc::c_ulong {
...
unsafe extern "C" fn BrotliCompressBufferQuality10(
    ...
    if 0 as libc::c_int != 0 {
        current_block = 7927247141152969957;
    } else {
...
unsafe extern "C" fn InjectBytePaddingBlock(mut s: *mut BrotliEncoderState) {
    ...
    *destination.offset(0 as libc::c_int as isize) = seal as uint8_t;
    ...
    (*s)
        .available_out_ = ((*s).available_out_ as libc::c_ulong)
        .wrapping_add(
...
unsafe extern "C" fn InjectFlushOrPushOutput(
        ...
        && *available_out != 0 as libc::c_int as libc::c_ulong
    {
        ...
        *available_out = (*available_out as libc::c_ulong).wrapping_sub(copy_output_size)
        ...
        (*s)
            .available_out_ = ((*s).available_out_ as libc::c_ulong)
            .wrapping_sub(copy_output_size) as size_t as size_t;
        (*s)
            .total_out_ = ((*s).total_out_ as libc::c_ulong)
            .wrapping_add(copy_output_size) as size_t as size_t;
            ...
            *storage
            ...
            if inplace != 0 {
                ...
                *available_out = (*available_out as libc::c_ulong)
                ...
                (*s)
                    .total_out_ = ((*s).total_out_ as libc::c_ulong)
                    .wrapping_add(out_bytes) as size_t as size_t;
                if !total_out.is_null() {
                    *total_out = (*s).total_out_;
                }
            } else {
            ...
            == BROTLI_STREAM_METADATA_HEAD as libc::c_int as libc::c_uint
        {
            ...
            *available_in = (*available_in as libc::c_ulong)
            ...
            (*s)
                .remaining_metadata_bytes_ = ((*s).remaining_metadata_bytes_
                as libc::c_uint)
                .wrapping_sub(copy_0) as uint32_t as uint32_t;
...
pub unsafe extern "C" fn BrotliEncoderTakeOutput(
    ...
    if consumed_size != 0 {
        ...
        (*s)
            .available_out_ = ((*s).available_out_ as libc::c_ulong)
            .wrapping_sub(consumed_size) as size_t as size_t;
        (*s)
            .total_out_ = ((*s).total_out_ as libc::c_ulong).wrapping_add(consumed_size)
            as size_t as size_t;
        CheckFlushComplete(s);
        *size = consumed_size;
    } else {
