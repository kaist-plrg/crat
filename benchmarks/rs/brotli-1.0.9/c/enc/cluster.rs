...
pub unsafe extern "C" fn BrotliCompareAndPushToQueueCommand(
        ...
        == 0 as libc::c_int as libc::c_ulong
    {
        ...
        let mut threshold: libc::c_double = if *num_pairs
            == 0 as libc::c_int as libc::c_ulong
        {
            1e99f64
        } else {
...
pub unsafe extern "C" fn BrotliHistogramReindexDistance(
    ...
    let mut new_index: *mut uint32_t = if length > 0 as libc::c_int as libc::c_ulong {
        BrotliAllocate(
            m,
...
pub unsafe extern "C" fn BrotliClusterHistogramsLiteral(
    ...
    let mut cluster_size: *mut uint32_t = if in_size > 0 as libc::c_int as libc::c_ulong
    {
        BrotliAllocate(
            m,
    ...
    let mut clusters: *mut uint32_t = if in_size > 0 as libc::c_int as libc::c_ulong {
        BrotliAllocate(
            m,
            in_size.wrapping_mul(::std::mem::size_of::<uint32_t>() as libc::c_ulong),
        ) as *mut uint32_t
    ...
    if pairs_capacity < max_num_pairs.wrapping_add(1 as libc::c_int as libc::c_ulong) {
        let mut _new_size: size_t = if pairs_capacity
            == 0 as libc::c_int as libc::c_ulong
        {
