...
unsafe extern "C" fn HistogramAddHistogramLiteral(
    ...
    let mut i: size_t = 0;
    (*self_0)
        .total_count_ = ((*self_0).total_count_ as libc::c_ulong)
        .wrapping_add((*v).total_count_) as size_t as size_t;
    i = 0 as libc::c_int as size_t;
    while i < 256 as libc::c_int as libc::c_ulong {
        (*self_0)
            .data_[i
            as usize] = ((*self_0).data_[i as usize] as libc::c_uint)
            .wrapping_add((*v).data_[i as usize]) as uint32_t as uint32_t;
        i = i.wrapping_add(1);
        i;
    }
}
...
unsafe extern "C" fn HistogramAddVectorLiteral(
    ...
    (*self_0)
        .total_count_ = ((*self_0).total_count_ as libc::c_ulong).wrapping_add(n)
        as size_t as size_t;
...
unsafe extern "C" fn FindBlocksCommand(
    ...
    let mut byte_ix_0: size_t = length.wrapping_sub(1 as libc::c_int as libc::c_ulong);
            ...
            *cost.offset(k as isize) -= min_cost;
            if *cost.offset(k as isize) >= block_switch_cost {
                let mask: uint8_t = ((1 as libc::c_uint)
                    << (k & 7 as libc::c_int as libc::c_ulong)) as uint8_t;
                *cost.offset(k as isize) = block_switch_cost;
                let ref mut fresh4 = *switch_signal
                    .offset(ix.wrapping_add(k >> 3 as libc::c_int) as isize);
                *fresh4 = (*fresh4 as libc::c_int | mask as libc::c_int) as uint8_t;
            }
            k = k.wrapping_add(1);
            k;
        }
            ...
            *cost.offset(k as isize) -= min_cost;
            if *cost.offset(k as isize) >= block_switch_cost {
                let mask: uint8_t = ((1 as libc::c_uint)
                    << (k & 7 as libc::c_int as libc::c_ulong)) as uint8_t;
                *cost.offset(k as isize) = block_switch_cost;
                let ref mut fresh5 = *switch_signal
                    .offset(ix.wrapping_add(k >> 3 as libc::c_int) as isize);
                *fresh5 = (*fresh5 as libc::c_int | mask as libc::c_int) as uint8_t;
            }
            k = k.wrapping_add(1);
            k;
        }
...
unsafe extern "C" fn RemapBlockIdsCommand(
    ...
    while i < length {
            ...
            == kInvalidId as libc::c_int
        {
            ...
            *new_id.offset(*block_ids.offset(i as isize) as isize) = fresh6;
        }
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn RemapBlockIdsDistance(
    ...
    while i < length {
            ...
            == kInvalidId as libc::c_int
        {
            let fresh8 = next_id;
            next_id = next_id.wrapping_add(1);
            *new_id.offset(*block_ids.offset(i as isize) as isize) = fresh8;
        }
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn ClusterBlocksCommand(
    ...
    let mut histogram_symbols: *mut uint32_t = if num_blocks
    ...
    let mut block_lengths: *mut uint32_t = if num_blocks
        > 0 as libc::c_int as libc::c_ulong
    {
        BrotliAllocate(
            m,
    ...
    i = 0 as libc::c_int as size_t;
    while i < length {
        let ref mut fresh9 = *block_lengths.offset(block_idx as isize);
        ...
        while j < num_to_combine {
            ...
            while k < *block_lengths.offset(i.wrapping_add(j) as isize) as libc::c_ulong
            {
                ...
                HistogramAddCommand(
                    ...
                    *data.offset(fresh10 as isize) as size_t,
                );
                k = k.wrapping_add(1);
                k;
            }
    ...
    while i < num_blocks {
        ...
        while j_0 < *block_lengths.offset(i as isize) as libc::c_ulong {
            let fresh13 = pos;
            pos = pos.wrapping_add(1);
            HistogramAddCommand(&mut histo, *data.offset(fresh13 as isize) as size_t);
        ...
        *histogram_symbols.offset(i as isize) = best_out;
        if *new_index.offset(best_out as isize) == kInvalidIndex {
            let fresh14 = next_index;
            next_index = next_index.wrapping_add(1);
            *new_index.offset(best_out as isize) = fresh14;
        }
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn ClusterBlocksDistance(
    ...
    i = 0 as libc::c_int as size_t;
    while i < length {
        let ref mut fresh15 = *block_lengths.offset(block_idx as isize);
    ...
    while i < num_blocks {
        ...
        while j < num_to_combine {
            ...
            while k < *block_lengths.offset(i.wrapping_add(j) as isize) as libc::c_ulong
            {
                ...
                HistogramAddDistance(
                    &mut *histograms.offset(j as isize),
                    *data.offset(fresh16 as isize) as size_t,
                );
                k = k.wrapping_add(1);
                k;
            }
        ...
        j = 0 as libc::c_int as size_t;
        while j < num_new_clusters {
            let fresh17 = all_histograms_size;
            all_histograms_size = all_histograms_size.wrapping_add(1);
            *all_histograms
                .offset(
                    fresh17 as isize,
                ) = *histograms.offset(new_clusters[j as usize] as isize);
            let fresh18 = cluster_size_size;
            cluster_size_size = cluster_size_size.wrapping_add(1);
            *cluster_size
                .offset(fresh18 as isize) = sizes[new_clusters[j as usize] as usize];
            remap[new_clusters[j as usize] as usize] = j as uint32_t;
            j = j.wrapping_add(1);
            j;
        }
    ...
    while i < num_blocks {
        ...
        while j_0 < *block_lengths.offset(i as isize) as libc::c_ulong {
            let fresh19 = pos;
            pos = pos.wrapping_add(1);
            HistogramAddDistance(&mut histo, *data.offset(fresh19 as isize) as size_t);
            j_0 = j_0.wrapping_add(1);
            j_0;
        }
        best_out = if i == 0 as libc::c_int as libc::c_ulong {
        ...
        *histogram_symbols.offset(i as isize) = best_out;
        if *new_index.offset(best_out as isize) == kInvalidIndex {
            let fresh20 = next_index;
            next_index = next_index.wrapping_add(1);
            *new_index.offset(best_out as isize) = fresh20;
        }
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn ClusterBlocksLiteral(
    ...
    while i < num_blocks {
        ...
        while j_0 < *block_lengths.offset(i as isize) as libc::c_ulong {
            let fresh25 = pos;
            pos = pos.wrapping_add(1);
            HistogramAddLiteral(&mut histo, *data.offset(fresh25 as isize) as size_t);
            j_0 = j_0.wrapping_add(1);
            j_0;
        }
        best_out = if i == 0 as libc::c_int as libc::c_ulong {
...
unsafe extern "C" fn SplitByteVectorDistance(
    ...
    let mut block_ids: *mut uint8_t = if length > 0 as libc::c_int as libc::c_ulong {
        BrotliAllocate(
            m,
    ...
    let bitmaplen: size_t = num_histograms
        .wrapping_add(7 as libc::c_int as libc::c_ulong) >> 3 as libc::c_int;
    let mut insert_cost: *mut libc::c_double = if data_size.wrapping_mul(num_histograms)
        > 0 as libc::c_int as libc::c_ulong
    {
        BrotliAllocate(
            m,
    ...
    let mut cost: *mut libc::c_double = if num_histograms
        > 0 as libc::c_int as libc::c_ulong
    {
        BrotliAllocate(
            m,
    ...
    let mut switch_signal: *mut uint8_t = if length.wrapping_mul(bitmaplen)
        > 0 as libc::c_int as libc::c_ulong
    {
        BrotliAllocate(
            m,
    ...
    let mut new_id: *mut uint16_t = if num_histograms > 0 as libc::c_int as libc::c_ulong
    {
        BrotliAllocate(
            m,
            num_histograms
...
pub unsafe extern "C" fn BrotliSplitBlock(
    ...
    let mut literals: *mut uint8_t = if literals_count
        > 0 as libc::c_int as libc::c_ulong
    {
        BrotliAllocate(
            m,
