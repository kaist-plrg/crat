...
pub unsafe extern "C" fn snoopy_util_pwd_convertUidToUsername(
    mut uid: uid_t,
) -> *mut libc::c_char {
    ...
    let mut pwd_uid: *mut passwd = 0 as *mut passwd;
    let mut buffpwd_uid: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut buffpwdsize_uid: libc::c_long = 0 as libc::c_int as libc::c_long;
    let mut username: *mut libc::c_char = 0 as *mut libc::c_char;
    buffpwdsize_uid = sysconf(_SC_GETPW_R_SIZE_MAX as libc::c_int);
    if -(1 as libc::c_int) as libc::c_long == buffpwdsize_uid {
        buffpwdsize_uid = 16384 as libc::c_int as libc::c_long;
    }
    buffpwd_uid = malloc(buffpwdsize_uid as libc::c_ulong) as *mut libc::c_char;
    if buffpwd_uid.is_null() {
        return 0 as *mut libc::c_char;
    }
    username = malloc((256 as libc::c_int + 1 as libc::c_int) as libc::c_ulong)
        as *mut libc::c_char;
    if username.is_null() {
        free(buffpwd_uid as *mut libc::c_void);
        return 0 as *mut libc::c_char;
    }
    *username.offset(0 as libc::c_int as isize) = '\0' as i32 as libc::c_char;
    if 0 as libc::c_int
        != getpwuid_r(
            uid,
            &mut pwd,
            buffpwd_uid,
            buffpwdsize_uid as size_t,
            &mut pwd_uid,
        )
    {
        free(buffpwd_uid as *mut libc::c_void);
    ...
    if pwd_uid.is_null() {
        snprintf(
            ...
            b"user-%d\0" as *const u8 as *const libc::c_char,
            uid as libc::c_int,
        );
    } else {
        snprintf(
    ...
    *username.offset(256 as libc::c_int as isize) = '\0' as i32 as libc::c_char;
    free(buffpwd_uid as *mut libc::c_void);
    return username;
}
