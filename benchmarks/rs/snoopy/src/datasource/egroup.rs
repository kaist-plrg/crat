...
pub unsafe extern "C" fn snoopy_datasource_egroup(
    ...
    let mut gr_gid: *mut group = 0 as *mut group;
    let mut buffgr_gid: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut buffgrsize_gid: libc::c_long = 0 as libc::c_int as libc::c_long;
    let mut messageLength: libc::c_int = 0 as libc::c_int;
    buffgrsize_gid = sysconf(_SC_GETGR_R_SIZE_MAX as libc::c_int);
    if -(1 as libc::c_int) as libc::c_long == buffgrsize_gid {
        buffgrsize_gid = 16384 as libc::c_int as libc::c_long;
    }
    buffgr_gid = malloc(buffgrsize_gid as libc::c_ulong) as *mut libc::c_char;
    if buffgr_gid.is_null() {
        return snprintf(
            result,
        ...
        )
    {
        messageLength = snprintf(
            result,
            2048 as libc::c_int as libc::c_ulong,
            b"ERROR(getgrgid_r)\0" as *const u8 as *const libc::c_char,
        );
    } else if gr_gid.is_null() {
        messageLength = snprintf(
            result,
            2048 as libc::c_int as libc::c_ulong,
    ...
    free(buffgr_gid as *mut libc::c_void);
