...
pub unsafe extern "C" fn snoopy_datasource_eusername(
    ...
    let mut messageLength: libc::c_int = 0 as libc::c_int;
    ...
    if buffpwd_uid.is_null() {
        return snprintf(
            ...
            b"ERROR(malloc)\0" as *const u8 as *const libc::c_char,
        );
    }
    if 0 as libc::c_int
        != getpwuid_r(
        ...
        )
    {
        messageLength = snprintf(
            result,
            2048 as libc::c_int as libc::c_ulong,
            b"ERROR(getpwuid_r)\0" as *const u8 as *const libc::c_char,
        );
    } else if pwd_uid.is_null() {
        messageLength = snprintf(
            result,
            2048 as libc::c_int as libc::c_ulong,
