    ...
    while cur_idx < (*cur_nodes).nelem {
        ...
        if (*((*dfa).nodes).offset(cur_node as isize)).accept_mb() != 0 {
            ...
            if naccepted > 1 as libc::c_int {
                ...
                let ref mut fresh70 = *((*mctx).state_log).offset(next_idx as isize);
                *fresh70 = re_acquire_state(&mut err, dfa, &mut union_set);
                    ...
                    if !((*entry).node != node) {
                            ...
                            ) as libc::c_int != 0)
                        {
                            ...
                            if !ok as libc::c_int as libc::c_long != 0 {
                                ...
                                let ref mut fresh71 = *(local_sctx.sifted_states)
                                    .offset(str_idx as isize);
                                *fresh71 = cur_state;
                ...
                if to_idx == cur_str {
                    ...
                    match current_block {
                        ...
                        _ => {
                            ...
                            *fresh73 = re_acquire_state(&mut err, dfa, &mut union_set);
                            rpl_free(union_set.elems as *mut libc::c_void);
                            if ((*((*mctx).state_log).offset(to_idx as isize)).is_null()
                                && err as libc::c_int != _REG_NOERROR as libc::c_int)
                                as libc::c_int as libc::c_long != 0
...
unsafe extern "C" fn merge_state_with_log(
    ...
    if cur_idx > (*mctx).state_log_top {
        ...
        *fresh77 = re_acquire_state_context(err, dfa, &mut next_nodes, context);
        next_state = *fresh77;
                                ...
                                *fresh78 = re_acquire_state_context(
                                    ...
                                    as libc::c_int as libc::c_long != 0
                                {
                                    ...
                                    let ref mut fresh79 = *((*mctx).state_log)
                                        .offset(dest_str_idx as isize);
                                    *fresh79 = re_acquire_state_context(
                                        &mut err,
                                        dfa,
                                        &mut dest_nodes,
                                        context,
...
unsafe extern "C" fn check_arrival(
        ...
        || !cur_state.is_null() && (*cur_state).has_backref() as libc::c_int != 0
    {
        ...
        let ref mut fresh80 = *((*mctx).state_log).offset(str_idx as isize);
        *fresh80 = cur_state;
    }
