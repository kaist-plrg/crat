...
pub unsafe extern "C" fn rpl_fts_open(
    ...
    if fts_palloc(
        ...
        if !(*argv).is_null() {
            ...
            if parent.is_null() {
                current_block = 15961035062207988253;
            } else {
        ...
        match current_block {
            17788412896529399552 => {
                ...
                loop {
                    ...
                    if p.is_null() {
                        current_block = 3125350758098317472;
                        break;
                    }
...
pub unsafe extern "C" fn rpl_fts_read(mut sp: *mut FTS) -> *mut FTSENT {
            ...
            || (*p).fts_info as libc::c_int == 13 as libc::c_int)
    {
        ...
        if (*p).fts_info as libc::c_int == 1 as libc::c_int {
            ...
            p = (*sp).fts_child;
            (*sp).fts_child = 0 as *mut _ftsent;
            current_block = 11866784473395354086;
        } else {
            loop {
                tmp = p;
                if ((*p).fts_link).is_null() && !((*(*p).fts_parent).fts_dirp).is_null()
                {
                    ...
                    *((*sp).fts_path)
                    ...
                    p = (*p).fts_link;
                    if p.is_null() {
                        current_block = 15940684936200374155;
                        break;
                    }
                    (*sp).fts_cur = p;
                    ...
                    if (*p).fts_level == 0 as libc::c_int as libc::c_long {
                        ...
                        current_block = 17874579062798441615;
                        break;
                    } else {
            ...
            match current_block {
                17874579062798441615 => {}
                11866784473395354086 => {}
                _ => {
                    match current_block {
                        9353995356876505083 => {
                            rpl_free(tmp as *mut libc::c_void);
            ...
            if !p.is_null() {
                if d_namelen >= maxlen {
                    ...
                    if !fts_palloc(
                        ...
                        current_block_155 = 15036573363345641781;
                    } else {
...
unsafe extern "C" fn fts_safe_changedir(
                ...
                == 0 as libc::c_int
    {
        ...
        if fstat(newfd, &mut sb) != 0 {
            ret = -(1 as libc::c_int);
            current_block = 16994581725161548484;
