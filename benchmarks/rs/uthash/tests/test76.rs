...
unsafe fn main_0() -> libc::c_int {
    ...
    let mut V_StartPos: size_t = 0 as libc::c_int as size_t;
    ...
    *((*t).d).offset((*t).i as isize) = '\0' as i32 as libc::c_char;
    ...
    if !V_KMP_Table.is_null() {
        _utstring_BuildTable((*t).d, (*t).i, V_KMP_Table);
        loop {
            V_FindPos = _utstring_find(
                ((*s).d).offset(V_StartPos as isize),
                ((*s).i).wrapping_sub(V_StartPos),
                (*t).d,
                (*t).i,
                V_KMP_Table,
            );
            if V_FindPos >= 0 as libc::c_int as libc::c_long {
                V_FindPos = (V_FindPos as libc::c_ulong).wrapping_add(V_StartPos)
