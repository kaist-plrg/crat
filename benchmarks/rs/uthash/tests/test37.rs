...
unsafe fn main_0() -> libc::c_int {
    ...
    while i < 10 as libc::c_int {
        ...
        match _hj_k {
            11 => {
                ...
                current_block_57 = 1744910570668747089;
            }
            10 => {
                current_block_57 = 1744910570668747089;
            }
            9 => {
                current_block_57 = 17301571373625706371;
            }
            8 => {
                current_block_57 = 17911383226362114466;
            }
            7 => {
                current_block_57 = 6881611655122192447;
            }
            6 => {
                current_block_57 = 5357406655149892490;
            }
            5 => {
                current_block_57 = 16734287195833850954;
            }
            4 => {
                current_block_57 = 5801928789528630000;
            }
            3 => {
                current_block_57 = 17404231275461091374;
            }
            2 => {
                current_block_57 = 360859195070300319;
            }
            1 => {
                current_block_57 = 2989611358996265166;
            }
            _ => {
    ...
    printf(
        ...
        if !users.is_null() { (*(*users).hh.tbl).num_items } else { 0 as libc::c_uint },
    );
        ...
        while _src_bkt < (*(*users).hh.tbl).num_buckets {
            ...
            while !_src_hh.is_null() {
                _elt = (_src_hh as *mut libc::c_char)
                    .offset(-((*(*users).hh.tbl).hho as isize)) as *mut libc::c_void;
                if (*(_elt as *mut example_user_t)).id % 2 as libc::c_int
                    == 0 as libc::c_int
                {
    ...
    printf(
        b"ausers count: %u\n\0" as *const u8 as *const libc::c_char,
        if !ausers.is_null() { (*(*ausers).ah.tbl).num_items } else { 0 as libc::c_uint },
    );
    if !ausers.is_null() {
        free((*(*ausers).ah.tbl).buckets as *mut libc::c_void);
        free((*ausers).ah.tbl as *mut libc::c_void);
        ausers = 0 as *mut example_user_t;
    }
    printf(b"cleared ausers.\n\0" as *const u8 as *const libc::c_char);
    printf(
        b"ausers count: %u\n\0" as *const u8 as *const libc::c_char,
        if !ausers.is_null() { (*(*ausers).ah.tbl).num_items } else { 0 as libc::c_uint },
    );
