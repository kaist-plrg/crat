...
unsafe fn main_0() -> libc::c_int {
    ...
    while i < 10 as libc::c_int {
        ...
        match _hj_k {
            11 => {
                ...
                current_block_57 = 13263008818204521728;
            }
            10 => {
                current_block_57 = 13263008818204521728;
            }
            9 => {
                current_block_57 = 3570511084383332863;
            }
            8 => {
                current_block_57 = 7955134480489028529;
            }
            7 => {
                current_block_57 = 16275225790648504471;
            }
            6 => {
                current_block_57 = 13674089274984699776;
            }
            5 => {
                current_block_57 = 3179649331693791008;
            }
            4 => {
                current_block_57 = 15571966665014886142;
            }
            3 => {
                current_block_57 = 1198154044258105267;
            }
            2 => {
                current_block_57 = 11810296512306887234;
            }
            1 => {
                current_block_57 = 17889034543650248888;
            }
            _ => {
    ...
    c = if !users.is_null() { (*(*users).hh.tbl).num_items } else { 0 as libc::c_uint };
    printf(b"%u users. Deleting odd id's...\n\0" as *const u8 as *const libc::c_char, c);
    user = users;
    tmp = (if !users.is_null() { (*users).hh.next } else { 0 as *mut libc::c_void })
        as *mut example_user_t;
    while !user.is_null() {
        if (*user).id & 1 as libc::c_int != 0 as libc::c_int {
    ...
    c = if !users.is_null() { (*(*users).hh.tbl).num_items } else { 0 as libc::c_uint };
    printf(
        b"%u users. Deleting remaining id's...\n\0" as *const u8 as *const libc::c_char,
        c,
    );
    user = users;
    tmp = (if !users.is_null() { (*users).hh.next } else { 0 as *mut libc::c_void })
    ...
    while !user.is_null() {
        ...
        if ((*_hd_hh_del_0).prev).is_null() && ((*_hd_hh_del_0).next).is_null() {
            ...
            _hd_bkt_0 = (*_hd_hh_del_0).hashv
                & ((*(*users).hh.tbl).num_buckets).wrapping_sub(1 as libc::c_uint);
            let mut _hd_head_0: *mut UT_hash_bucket = &mut *((*(*users).hh.tbl).buckets)
