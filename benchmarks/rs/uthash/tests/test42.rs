...
unsafe extern "C" fn eltcmp(mut a: *mut el, mut b: *mut el) -> libc::c_int {
    return (*a).id - (*b).id;
}
unsafe fn main_0() -> libc::c_int {
    ...
    if !e.is_null() {
        printf(b"search scalar found b\n\0" as *const u8 as *const libc::c_char);
    }
    e = head;
    while !e.is_null() {
        if eltcmp(e, &mut *els.as_mut_ptr().offset(0 as libc::c_int as isize))
            == 0 as libc::c_int
        {
            break;
        }
    ...
    if !e.is_null() {
        printf(b"search found %c\n\0" as *const u8 as *const libc::c_char, (*e).id);
    }
    e = head;
    while !e.is_null()
        && {
        ...
        }
    {
        if !head.is_null() {} else {
            __assert_fail(
                ...
                b"test42.c\0" as *const u8 as *const libc::c_char,
