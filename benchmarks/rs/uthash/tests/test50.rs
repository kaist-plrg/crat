...
unsafe fn main_0() -> libc::c_int {
    ...
    let mut p: *mut libc::c_long = 0 as *mut libc::c_long;
    let mut long_icd: UT_icd = {
        let mut init = UT_icd {
            sz: ::std::mem::size_of::<libc::c_long>() as libc::c_ulong,
            init: None,
            copy: None,
            dtor: None,
        };
    ...
    if ((*nums).icd.copy).is_some() {
        ...
        ((*nums).icd.copy)
            .unwrap()(
            ((*nums).d)
                .offset(((*nums).icd.sz).wrapping_mul(fresh0 as libc::c_ulong) as isize)
                as *mut libc::c_void,
            &mut l as *mut libc::c_long as *const libc::c_void,
        );
    } else {
        let fresh1 = (*nums).i;
        (*nums).i = ((*nums).i).wrapping_add(1);
        memcpy(
            ((*nums).d)
                .offset(((*nums).icd.sz).wrapping_mul(fresh1 as libc::c_ulong) as isize)
                as *mut libc::c_void,
            &mut l as *mut libc::c_long as *const libc::c_void,
            (*nums).icd.sz,
        );
    ...
    if ((*nums).icd.copy).is_some() {
        ...
        ((*nums).icd.copy)
            .unwrap()(
            ((*nums).d)
                .offset(((*nums).icd.sz).wrapping_mul(fresh2 as libc::c_ulong) as isize)
                as *mut libc::c_void,
            &mut l as *mut libc::c_long as *const libc::c_void,
        );
    } else {
        ...
        memcpy(
            ((*nums).d)
                .offset(((*nums).icd.sz).wrapping_mul(fresh3 as libc::c_ulong) as isize)
                as *mut libc::c_void,
            &mut l as *mut libc::c_long as *const libc::c_void,
            (*nums).icd.sz,
        );
    }
    p = 0 as *mut libc::c_long;
    loop {
        p = (if p.is_null() {
            if (*nums).i != 0 {
                ((*nums).d)
                    .offset(
                        ((*nums).icd.sz).wrapping_mul(0 as libc::c_int as libc::c_ulong)
                            as isize,
                    ) as *mut libc::c_void
            } else {
                0 as *mut libc::c_void
            }
        } else if (*nums).i as libc::c_ulong
            != ((p as *mut libc::c_char).offset_from((*nums).d) as libc::c_long
                as libc::c_ulong)
