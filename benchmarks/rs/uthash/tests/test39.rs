...
unsafe fn main_0() -> libc::c_int {
    let mut keys: [*const libc::c_char; 3] = [
        b"eins\0" as *const u8 as *const libc::c_char,
        b"zwei\0" as *const u8 as *const libc::c_char,
        b"drei\0" as *const u8 as *const libc::c_char,
    ];
    let mut i: libc::c_uint = 0;
    let mut nsp: *mut ns_t = 0 as *mut ns_t;
    let mut head: *mut ns_t = 0 as *mut ns_t;
            ...
            .wrapping_div(::std::mem::size_of::<*const libc::c_char>() as libc::c_ulong)
    {
        ...
        nsp = malloc(::std::mem::size_of::<ns_t>() as libc::c_ulong) as *mut ns_t;
        if nsp.is_null() {
            exit(-(1 as libc::c_int));
        }
        (*nsp).name = keys[i as usize];
        let mut _ha_hashv: libc::c_uint = 0;
        let mut _hj_i: libc::c_uint = 0;
        let mut _hj_j: libc::c_uint = 0;
        let mut _hj_k: libc::c_uint = 0;
        let mut _hj_key: *const libc::c_uchar = (*nsp).name as *const libc::c_uchar;
        _ha_hashv = 0xfeedbeef as libc::c_uint;
        _hj_j = 0x9e3779b9 as libc::c_uint;
        _hj_i = _hj_j;
        _hj_k = strlen((*nsp).name) as libc::c_uint;
        ...
        _ha_hashv = _ha_hashv.wrapping_add(strlen((*nsp).name) as libc::c_uint);
        ...
        match _hj_k {
            ...
            _ => {
                current_block_58 = 14220266465818359136;
            }
        }
        ...
        _ha_hashv = _ha_hashv.wrapping_sub(_hj_j);
        _ha_hashv ^= _hj_j >> 15 as libc::c_int;
        (*nsp).hh.hashv = _ha_hashv;
        (*nsp).hh.key = (*nsp).name as *const libc::c_void;
        (*nsp).hh.keylen = strlen((*nsp).name) as libc::c_uint;
        if head.is_null() {
            (*nsp).hh.next = 0 as *mut libc::c_void;
            (*nsp).hh.prev = 0 as *mut libc::c_void;
            (*nsp)
                .hh
                .tbl = malloc(::std::mem::size_of::<UT_hash_table>() as libc::c_ulong)
                as *mut UT_hash_table;
            if ((*nsp).hh.tbl).is_null() {
                exit(-(1 as libc::c_int));
            } else {
                memset(
                    (*nsp).hh.tbl as *mut libc::c_void,
                    '\0' as i32,
                    ::std::mem::size_of::<UT_hash_table>() as libc::c_ulong,
                );
                (*(*nsp).hh.tbl).tail = &mut (*nsp).hh;
                (*(*nsp).hh.tbl).num_buckets = 32 as libc::c_uint;
                (*(*nsp).hh.tbl).log2_num_buckets = 5 as libc::c_uint;
                (*(*nsp).hh.tbl)
                    .hho = (&mut (*nsp).hh as *mut UT_hash_handle as *mut libc::c_char)
                    .offset_from(nsp as *mut libc::c_char) as libc::c_long;
                (*(*nsp).hh.tbl)
                    .buckets = malloc(
                ...
                (*(*nsp).hh.tbl).signature = 0xa0111fe1 as libc::c_uint;
                if ((*(*nsp).hh.tbl).buckets).is_null() {
                    exit(-(1 as libc::c_int));
                } else {
                    memset(
                        (*(*nsp).hh.tbl).buckets as *mut libc::c_void,
                        '\0' as i32,
                        (32 as libc::c_uint as libc::c_ulong)
                            .wrapping_mul(
                                ::std::mem::size_of::<UT_hash_bucket>() as libc::c_ulong,
            ...
            (*nsp).hh.tbl = (*head).hh.tbl;
            (*nsp).hh.next = 0 as *mut libc::c_void;
            (*nsp)
                .hh
                .prev = ((*(*head).hh.tbl).tail as *mut libc::c_char)
                .offset(-((*(*head).hh.tbl).hho as isize)) as *mut libc::c_void;
            (*(*(*head).hh.tbl).tail).next = nsp as *mut libc::c_void;
            (*(*head).hh.tbl).tail = &mut (*nsp).hh;
        }
        ...
        (*_ha_head).count;
        (*nsp).hh.hh_next = (*_ha_head).hh_head;
        (*nsp).hh.hh_prev = 0 as *mut UT_hash_handle;
        if !((*_ha_head).hh_head).is_null() {
            (*(*_ha_head).hh_head).hh_prev = &mut (*nsp).hh;
        }
        (*_ha_head).hh_head = &mut (*nsp).hh;
        if (*_ha_head).count
            >= ((*_ha_head).expand_mult)
                .wrapping_add(1 as libc::c_uint)
                .wrapping_mul(10 as libc::c_uint) && (*(*nsp).hh.tbl).noexpand == 0
        {
            ...
            _he_new_buckets = malloc(
                (::std::mem::size_of::<UT_hash_bucket>() as libc::c_ulong)
                    .wrapping_mul((*(*nsp).hh.tbl).num_buckets as libc::c_ulong)
            ...
            if _he_new_buckets.is_null() {
                ...
                (*(*nsp).hh.tbl)
                    .ideal_chain_maxlen = ((*(*nsp).hh.tbl).num_items
                    >> ((*(*nsp).hh.tbl).log2_num_buckets)
                        .wrapping_add(1 as libc::c_uint))
                    .wrapping_add(
                        (if (*(*nsp).hh.tbl).num_items
                            & ((*(*nsp).hh.tbl).num_buckets)
                ...
                (*(*nsp).hh.tbl).nonideal_items = 0 as libc::c_int as libc::c_uint;
                _he_bkt_i = 0 as libc::c_int as libc::c_uint;
                while _he_bkt_i < (*(*nsp).hh.tbl).num_buckets {
                    _he_thh = (*((*(*nsp).hh.tbl).buckets).offset(_he_bkt_i as isize))
                        .hh_head;
                    while !_he_thh.is_null() {
                        ...
                        if (*_he_newbkt).count > (*(*nsp).hh.tbl).ideal_chain_maxlen {
                            (*(*nsp).hh.tbl)
                                .nonideal_items = ((*(*nsp).hh.tbl).nonideal_items)
                                .wrapping_add(1);
                            (*(*nsp).hh.tbl).nonideal_items;
                            if (*_he_newbkt).count
                                > ((*_he_newbkt).expand_mult)
                                    .wrapping_mul((*(*nsp).hh.tbl).ideal_chain_maxlen)
                            {
                                (*_he_newbkt)
                ...
                free((*(*nsp).hh.tbl).buckets as *mut libc::c_void);
                (*(*nsp).hh.tbl)
                    .num_buckets = ((*(*nsp).hh.tbl).num_buckets)
                    .wrapping_mul(2 as libc::c_uint);
                (*(*nsp).hh.tbl)
                    .log2_num_buckets = ((*(*nsp).hh.tbl).log2_num_buckets)
                    .wrapping_add(1);
                (*(*nsp).hh.tbl).log2_num_buckets;
                (*(*nsp).hh.tbl).buckets = _he_new_buckets;
                (*(*nsp).hh.tbl)
                    .ineff_expands = if (*(*nsp).hh.tbl).nonideal_items
                    > (*(*nsp).hh.tbl).num_items >> 1 as libc::c_int
                {
                    ((*(*nsp).hh.tbl).ineff_expands).wrapping_add(1 as libc::c_uint)
                } else {
                    0 as libc::c_uint
                };
                if (*(*nsp).hh.tbl).ineff_expands > 1 as libc::c_uint {
                    (*(*nsp).hh.tbl).noexpand = 1 as libc::c_int as libc::c_uint;
                }
            }
        }
        i = i.wrapping_add(1);
    ...
    printf(
        b"hash count is %u\n\0" as *const u8 as *const libc::c_char,
        if !head.is_null() { (*(*head).hh.tbl).num_items } else { 0 as libc::c_uint },
    );
    i = 0 as libc::c_int as libc::c_uint;
    while (i as libc::c_ulong)
        < (::std::mem::size_of::<[*const libc::c_char; 3]>() as libc::c_ulong)
    ...
    {
        printf(
            b"looking for key %s... \0" as *const u8 as *const libc::c_char,
            keys[i as usize],
        );
        nsp = 0 as *mut ns_t;
        if !head.is_null() {
            ...
            let mut _hj_key_0: *const libc::c_uchar = keys[i as usize]
                as *const libc::c_uchar;
            _hf_hashv = 0xfeedbeef as libc::c_uint;
            _hj_j_0 = 0x9e3779b9 as libc::c_uint;
            _hj_i_0 = _hj_j_0;
            _hj_k_0 = strlen(keys[i as usize]) as libc::c_uint;
            ...
            _hf_hashv = _hf_hashv.wrapping_add(strlen(keys[i as usize]) as libc::c_uint);
            ...
            match _hj_k_0 {
                11 => {
                    ...
                    current_block_255 = 15577564057197530261;
                }
                10 => {
                    current_block_255 = 15577564057197530261;
                }
                9 => {
                    current_block_255 = 16190753031682270451;
                }
                8 => {
                    current_block_255 = 5433040671696334990;
                }
                7 => {
                    current_block_255 = 4042466557151416159;
                }
                6 => {
                    current_block_255 = 16253746283936965345;
                }
                5 => {
                    current_block_255 = 4111030357467383836;
                }
                4 => {
                    current_block_255 = 10061427371901828084;
                }
                3 => {
                    current_block_255 = 4069473758053909909;
                }
                2 => {
                    current_block_255 = 6573832249731052516;
                }
                1 => {
                    current_block_255 = 6477219604861949104;
                }
                _ => {
                    current_block_255 = 2102591061404402436;
                }
            }
            ...
            if !head.is_null() {
                ...
                if 1 as libc::c_int != 0 as libc::c_int {
                    if !((*((*(*head).hh.tbl).buckets).offset(_hf_bkt as isize)).hh_head)
                        .is_null()
                    {
                        nsp = ((*((*(*head).hh.tbl).buckets).offset(_hf_bkt as isize))
                            .hh_head as *mut libc::c_char)
                            .offset(-((*(*head).hh.tbl).hho as isize))
                            as *mut libc::c_void as *mut ns_t;
                    } else {
                        nsp = 0 as *mut ns_t;
                    }
                    while !nsp.is_null() {
                        if (*nsp).hh.hashv == _hf_hashv
                            && (*nsp).hh.keylen as libc::c_ulong
                                == strlen(keys[i as usize])
                        {
                            if memcmp(
                                (*nsp).hh.key,
                                keys[i as usize] as *const libc::c_void,
                                strlen(keys[i as usize]),
                            ) == 0 as libc::c_int
                            {
                                break;
                        ...
                        if !((*nsp).hh.hh_next).is_null() {
                            nsp = ((*nsp).hh.hh_next as *mut libc::c_char)
                                .offset(-((*(*head).hh.tbl).hho as isize))
                                as *mut libc::c_void as *mut ns_t;
                        } else {
                            nsp = 0 as *mut ns_t;
