...
unsafe fn main_0() -> libc::c_int {
    ...
    printf(
        b"deleting head %shead->prev: %s\0" as *const u8 as *const libc::c_char,
        ((*head).bname).as_mut_ptr(),
        ((*(*head).prev).bname).as_mut_ptr(),
    );
    if !head.is_null() {} else {
        __assert_fail(
            b"(head) != NULL\0" as *const u8 as *const libc::c_char,
            b"test29.c\0" as *const u8 as *const libc::c_char,
    ...
    if !((*head).prev).is_null() {} else {
        __assert_fail(
            b"(head)->prev != NULL\0" as *const u8 as *const libc::c_char,
            b"test29.c\0" as *const u8 as *const libc::c_char,
            49 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 11], &[libc::c_char; 11]>(b"int main()\0"))
                .as_ptr(),
