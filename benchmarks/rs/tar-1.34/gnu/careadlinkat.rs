...
unsafe extern "C" fn readlink_stk(
    ...
    while !buf.is_null() {
        let mut link_length: ssize_t = preadlinkat.unwrap()(fd, filename, buf, buf_size);
        ...
        if link_length < 0 as libc::c_int as libc::c_long {
            if buf != buffer {
                ...
                ((*alloc).free).unwrap()(buf as *mut libc::c_void);
        ...
        if link_size < buf_size {
            ...
            *buf.offset(fresh0 as isize) = '\0' as i32 as libc::c_char;
            if buf == stack_buf {
                let mut b: *mut libc::c_char = ((*alloc).allocate).unwrap()(link_size)
                    as *mut libc::c_char;
                buf_size = link_size;
                if b.is_null() {
                    break;
                    ...
                    && ((*alloc).reallocate).is_some()
                {
                    let mut b_0: *mut libc::c_char = ((*alloc).reallocate)
                        .unwrap()(buf as *mut libc::c_void, link_size)
                        as *mut libc::c_char;
                    if !b_0.is_null() {
                        return b_0;
                    }
            ...
            buf = ((*alloc).allocate).unwrap()(buf_size) as *mut libc::c_char;
        }
    }
    if ((*alloc).die).is_some() {
        ((*alloc).die).unwrap()(buf_size);
    }
