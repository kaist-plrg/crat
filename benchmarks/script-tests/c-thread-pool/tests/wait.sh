...
function test_wait_each_job { #threads #jobs
	...
	compile src/wait.c
	realsecs=$(time_exec ./test_wait $2 $1 1)
	threshold=1.00 # in secs

	ret=$(python3 -c "print((abs($realsecs-$2))<=$threshold)")
...
function test_wait_pool { #threads #jobs
	...
	compile src/wait.c
	realsecs=$(time_exec ./test_wait $2 $1 0)
	threshold=1.00 # in secs
	
	expected_time=$(python3 -c "import math; print(math.ceil($2/$1.0))")
	ret=$(python3 -c "print((abs($realsecs-$expected_time))<=$threshold)")
